...
Semantic Diffは `JP1/AJS: Compare JP1/AJS Semantic Diff` から起動します。レポートの表示後、コピー操作を行うまでクリップボードを変更しません。
...
テレメトリ通信はテレメトリ用のアダプターを通じて行われます。CSVは、利用者がコピーまたは保存を選んだときに出力します。ファイルに保存する場合だけ、保存先を指定します。Semantic DiffのレポートはVS Code上の仮想Markdownとして表示され、表示中のMarkdownを明示的にコピーできます。レポートの保存も利用者の保存操作で行います。

WebAPI import betaを使う場合だけ、利用者が選択したJP1/AJS WebAPIエンドポイントへread-onlyの通信を行います。拡張機能の通常のローカル定義確認にWebAPI接続は必要ありません。

## 制約と既知の問題

- JP1/AJS製品バージョン、OS、未対応構文、最大ファイルサイズを網羅した対応表は、現在のリポジトリの根拠がないため掲載していません。利用する定義で表示結果を確認し、問題があればIssueへ報告してください。
...
本拡張機能は非公式のOSSです。株式会社日立製作所およびJP1製品の公式・公認ツールではありません。
