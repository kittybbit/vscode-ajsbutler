...
（`YYYY/MM/{weekday}`、`:n`、`:b`）を計算できます。完全修飾された相対日
（`YYYY/MM/+DD`、`+b`、相対曜日）も、`jc`または所属ジョブグループから
解決できる定義内の `sdd`、`md`、`stt=00:00` を使って計算します。登録時刻や
運用暦が必要な省略形、未解決の相対形、外部カレンダー情報は、引き続き未計算として明示します。

完全修飾された開所日・休業日形式（`YYYY/MM/*DD`、`*b[-DD]`、
`YYYY/MM/@DD`、`@b[-DD]`）も、`jc`または所属ジョブグループとその上位グループに
定義された `op`（開所日）／`cl`（休業日）だけを使って計算します。具体的な日付の
指定は曜日指定より優先され、必要な日が明示的に分類されていない場合は、ホストや外部の
カレンダーで補完せず未計算として明示します。

閉所日に対する置換（`sh=be`／`af`／`ca`）も、同じ定義内カレンダーを使って計算します。
`be`／`af` は開所日の検索範囲を `shd`（省略時は2日、明示時は1〜31日）以内に限定し、
`ca` は閉所日を実行なしにします。検索と比較期間の境界は最大31日までに限定します。
`sh=no`、不完全なカレンダー、`cy`／`cftd`を含むルール、カレンダーにない日付は、
サービス状態や外部カレンダーで補わず未計算として明示します。
