...
- 詳細ペインは選択を解除せず手動で折りたため、MUI `md`
  breakpoint 未満では初期表示時および狭幅への遷移時に自動で折りたたむ。
...
- FlowSelector から選択したノードへのグラフ移動は現在の zoom
  を維持してノードを中央表示し、検索結果への移動とは区別する。
- 検索結果の前後移動でも現在の zoom を維持して対象ノードを中央表示し、
  レイアウト変更時の全体 `fitView` とは区別する。
- ヘッダーは全幅の上段とし、その下のコンテンツ行に `FlowSelector`、グラフ、
  選択時の右詳細ペインを並べる。
- `FlowSelector` はコンテンツ行の左ペインとし、右詳細ペインと同様に手動折りたたみ、
  コンパクトなレール表示、狭幅時の自動折りたたみを提供する。
- ヘッダーは検索を最左の主要操作として配置し、単一項目しか持たない
  ハンバーガーメニューと左ツリーに重複するパンくず階層表示を削除する。
- パンくずを削除しても現在のグラフスコープを識別できる簡潔な表示は維持する。
...
Scenario: Move among multiple search results
  ...
  And the current zoom level is preserved
...
Scenario: Collapse selected-node context without clearing selection
  Given a selected node detail panel is visible
  When the viewport becomes narrow or the user collapses the panel
  Then the panel becomes a compact action rail
  And the selected node remains selected

Scenario: Center a tree-selected node without zooming
  Given a visible graph with a user-selected zoom level
  When the user selects an in-scope unit from the flow tree
  Then the graph centers the rendered node
  And the current zoom level is preserved
...
Scenario: Collapse the flow tree panel
  Given the flow tree is shown beside the graph
  When the viewport becomes narrow or the user collapses the tree panel
  Then the tree becomes a compact left action rail
  And the graph remains usable without overlaying the tree
  And the full-width header remains above the content row
...
- Multiple search results can be traversed forward and backward by buttons and
  Enter/Shift+Enter, with count and zoom-preserving viewport centering.
...
- The detail panel can collapse responsively or manually without clearing the
  selected node.
- Tree selection centers its rendered graph node without changing the current
  zoom level.
- Tree and graph selection and hover are synchronized without changing scope
  semantics.
- The flow tree behaves as a responsive or manually collapsible left panel
  aligned with the selected-node detail panel interaction model.
- The header spans the viewer above the FlowSelector, graph, and optional
  selected-node detail pane row.
- Header search is the leftmost primary control; redundant hamburger and
  breadcrumb controls are removed while current scope remains identifiable.
