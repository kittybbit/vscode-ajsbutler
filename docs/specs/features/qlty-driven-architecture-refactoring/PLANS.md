...
remaining unit-list helper findings (`getPriorityForUnitTypes`,
`buildUnitListGroup7View`) or remaining editor-feedback diagnostic helper
findings, then request separate approval before runtime work starts.

- Recent result:
  Slice-2-R preserved `parseSd` schedule-date projection while extracting
  schedule-date type detection and day formatting into local helpers. Targeted
  Qlty smell output no longer reports `parseSd`.
