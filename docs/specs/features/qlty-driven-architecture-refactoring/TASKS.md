...
- Active task:
  None.
- Runtime status:
  Slice-5-G expanded child growth offset application helper cleanup is
  complete.
- Use-case reflection:
  None required. Slice-5-G preserved expanded child growth offset propagation,
  affected right/down target selection, upper-panel intrusion handling, and
  deterministic `positionOverrides` behavior already covered by
...
Slice-5-F, and Slice-5-G refactors were behavior-preserving and do not
