...
- Mode: Replanning Mode for Slice 16 after the user questioned whether the
  shared MUI theme wrapper is necessary and requested standard MUI alignment;
  all fifteen predecessor implementation slices are complete and
...
- Final evidence is recorded below for the exact approved paths. Durable
  roadmap propagation remains a deferred closure proposal; the selected
  feature folder remains until Slice 16 and explicit Closure Approval.
...
- Replanning trigger: the user reported that the Semantic Diff Explorer MUI
  theme is not applied and explicitly requested the same resource/context
  mechanism used by the other webviews. Investigation found the production
  Explorer still observes DOM classes, attributes, and `matchMedia` through
  `semanticDiffExplorerThemeMode.ts`; its host panel rejects the shared
  `createViewerResourceRequest` before the custom semantic request validator.
...
- Replanning trigger: after Slice 8, the user asked whether the Semantic Diff
  presentation, VS Code host, Calendar host, and browser entrypoint paths follow
  the Flow/Unit List placement policy. Investigation shows browser entries are
  already thin `webview/editor` bootstrap files, while Calendar host files sit
  under the generic document-viewer `vscode/webview` package and Semantic Diff
  host roots retain three forwarding facades beside category packages. Slice 9
  defines one placement policy and moves only those feature-owned host/facade
  boundaries; pure Semantic Diff output modules remain in their host-neutral
  `presentation/semantic-diff` package and browser bundle names remain stable.
- Replanning trigger: after Slice 9, the user requested that the contents of
  `src/presentation/semantic-diff` follow the Flow/Unit List responsibility
  structure. Investigation shows fourteen files mix pure Markdown/JSON
  formatting, projection, localization, output-document aggregation, and a
  VS Code output-mode picker. The durable architecture assigns localization,
  report projections, and mode selection to Presentation, so the slice
  reorganizes host-neutral report work into a Presentation report subpackage,
  separates the VS Code chooser/document adapters, removes the two unused
  facade files, and retains the browser re-export cleanup as an approved
  implementation path. The current replan diff has no runtime entrypoint edit.
- Replanning trigger: after Slice 10, the user reported that Semantic Diff
  Explorer and Schedule Impact Calendar results are difficult to read as long
  strings and requested one consistent presentation. Investigation found
  existing MUI cards, alerts, lists, and theme/context wiring, but each view
  still owns duplicated section/card/key-value/status/empty markup; Calendar
  concatenates identifiers and details with `·`, while Explorer splits its
  summary, status, and tree detail presentation across view-specific wrappers.
  Slice 11 introduces generic browser-safe result primitives under the shared
...
- Replanning trigger: after Slice 11, the user reported that the shared period
  ...
  virtualization.
- Replanning trigger: after Slice 12, the user requested that repeated
  `label value label value` output become one independent label/value row per
  item in both Calendar and Explorer, including dense key/value lists nested
  inside `ResultComparison`. The shared `ResultKeyValueList` currently uses a
  flex-wrap dense mode and a parent two-column grid whose item containers can
  place adjacent pairs on one visual line. Slice 13 changes the shared row
  layout while preserving `<dl>/<dt>/<dd>` semantics, wrapping, responsive
  behavior, comparison side order, and all view-owned facts.
- Replanning trigger: the qlty smell inventory was compared at Slice 10
  (`0a5cdd31`), Slice 11 (`644161ca`), and Slice 12 (`2fb18daa`) against
  `origin/main`. Slice 11 introduced new findings in
  `SemanticDiffExplorerContents.tsx` (function complexity 5) and
  `ResultKeyValueList.tsx` (function complexity 10); Slice 12 introduced new
  findings in `semanticDiffExplorerTree.tsx` (total complexity 58,
  `detailItems` complexity 5, `scheduleDetails` complexity 9). The Calendar
  host/panel, Calendar model/view, and locale duplication findings already
  existed at the Slice 10 baseline and remain outside this cleanup. Slice 13
  removes only the newly introduced findings through small presentation
  helper extractions; it does not add qlty suppressions or change the qlty
  configuration.
- Replanning trigger: after Slice 14, the user requested a full UI audit of
  both the Semantic Diff Explorer and Schedule Impact Calendar. The Explorer
  header currently uses a transparent sticky AppBar and a native select, while
  Calendar filters use three independently styled native selects. The views
  also differ in sticky-surface treatment, panel opacity, menu sizing,
  in-menu headings, wrapping, spacing, elevation, borders, z-index, and
  responsive control behavior. Slice 15 groups actionable fixes around shared
  MUI surface and filter-control patterns, comparing the existing Flow/Table
  viewer surface and Unit Definition control conventions. It preserves the
  common resource/theme mechanism, result facts, filter semantics,
  keyboard/focus behavior, virtualization, and host contracts.
- Replanning trigger: after Slice 15 completed, the user questioned whether
  `muiTheme.ts` is needed and requested alignment with standard MUI theme
  configuration. The audit found that its `createSemanticDiffTheme` wrapper
  and several `semanticDiff*` exports are generic contracts consumed by
  Flow, Table, Explorer, Calendar, and shared controls, while
  `editor/shared/viewerThemeStyles.ts` carries overlapping VS Code token,
  focus, and high-contrast styles. Slice 16 keeps the one shared `createTheme`
  configuration because it preserves VS Code font variables, palette mode,
  44px interaction targets, and common MUI focus/control rules, but moves it
  into one generic `presentation/webview/shared/viewerTheme.ts`, removes the
  unused NativeSelect override and singleton, and migrates all consumers and
  tests without changing visual contracts or the common resource mechanism.
- The plan now has sixteen slices: pure comparison artifacts and sidecar
  ...
  shared viewer-resource theme/context correction for Explorer; the shared
  ...
  internal Calendar-ID presentation cleanup; the shared MUI presentation audit
  and filter/surface consistency fixes for Explorer and Calendar Slice 15; and
  the canonical shared MUI viewer-theme cleanup in Slice 16.
- Current boundaries remain: one identity pass and one schedule evaluation,
  ...
  messages, DTOs, or the common theme/resource mechanism. Slice 13 changes
  only shared browser-safe key/value row presentation and the named Explorer
  presentation helper organization. Slice 14 changes only Calendar
  presentation of internal identifiers and its user-visible labels. Slice 15
  changes only shared MUI presentation surfaces, filter controls, responsive
  wrapping, and related accessibility coverage. Slice 16 changes only the
  shared theme module/export names, duplicate global-style ownership, and
  corresponding imports/tests; it does not change theme/resource detection,
  view behavior, controls, DTOs, protocols, host lifecycle, or bundle names.
- Public calendar exposure is gated by the workflow's successful evaluated-
  ...
  Slice 3 implementation. Slices 1 through 15 are complete and committed on
...
- The original plan, replans, and Slice 4/5/6/7/8/9 packages have independent
  ...
  `0b914f48` are complete. All thirteen predecessor implementation reviews are
  `Ready` with
  ...
  complete. Slice 9 plan commit `65f123ee`, implementation review, automatic
  Completion Approval, and focused completion commit `bb8d7305` are complete.
...
- This file is the sole plan and current-state owner for this feature. Other
  ...
  scope.
- `docs/specs/roadmap.md` closure propagation removes the completed Wave 4
  calendar item while preserving the deferred schedule-semantics follow-ups.
  ...
  range and before/after comparison layout Slice 12 → one-row shared result
  alignment and targeted Slice 11/12 smell cleanup Slice 13 → internal
  Calendar-ID presentation cleanup Slice 14 → shared MUI presentation audit
  Slice 15 → canonical shared MUI viewer-theme cleanup Slice 16.
...
- Status: Slice 1 implementation is complete under approved third-replan
  ...
  implementation review is `Ready` with no Findings, automatic Completion
  Approval is recorded, and completion commit `fe042fb0` is complete.
- Planning scope: the internal application comparison-artifact contract and
  ...
  organization, shared result-presentation composition, localized
...
- Slice 12 plan review returned `Ready for approval` with no Findings and
  ...
  exact paths, focused plan commit `b25e38d5` is complete, implementation is
  complete under the approved paths, independent implementation review returned
  `Ready` with no Findings, automatic Completion Approval is recorded below,
...
- Human approval: The reviewed three-slice package, original internal Slice 1
  ...
  completion commit `fe042fb0` is complete. Slice 9 Human Plan Approval is
  recorded below on 2026-09-19 under the user's automatic no-findings
  instruction; focused plan commit `65f123ee` is complete. Slice 9
  implementation review is `Ready` with no Findings, automatic Completion
  ...
  recorded below; focused plan commit `b25e38d5` is complete, implementation is
  complete under the approved paths; independent implementation review
  returned `Ready` with no Findings, automatic Completion Approval is recorded,
  and focused completion commit `2fb18daa` is complete. Slice 13 plan approval
  is recorded,
...
- Slice order: Slice 1, Slice 2, Slice 3, Slice 4, Slice 5, Slice 6, Slice 7,
  ...
  Completion Approval, and focused commit after the plan gate; Feature Exit
  follows Slice 16.
...
- Status: Superseded by the approved replan result below.
- Trigger: the user explicitly requested that the remaining calendar host files,
  ...
  import relocation only; the earlier completed slices and the deferred closure
...
- Review route: the targeted replan was independently reviewed `Ready` with no
  Findings and approved in the result below.
...
- Commit status: Complete; focused replan commit `e51d6def`.
...
### Slice 7 Replan Human Approval Result

- Status: Approved
- Approved at: 2026-09-15; approved in current conversation
- Basis: independent `plan-reviewer` verdict Ready for approval; Findings none.
  Main applies the user's persistent automatic no-findings slice approval and
  explicit requirement to use the common webview theme mechanism exclusively.
- Approved scope: exactly Slice 7 removal of Explorer-specific theme detection,
  shared `MyAppContextProvider` resource integration, common host resource
  pre-dispatch, listed regressions and selected feature evidence below.
- Approved paths for this focused replan commit:
  - `docs/specs/features/schedule-impact-calendar/TASKS.md`
  - `docs/specs/features/schedule-impact-calendar/TRACEABILITY.md`
...
- Commit status: Complete; focused completed Slice 6 commit `c36ee1cf`.
...
### Slice 9 Replan Human Approval

- Status: Approved for the exact Semantic Diff placement-policy relocation
  below.
- Approved at: 2026-09-19; approved in the current conversation under the
  user's standing automatic no-findings slice approval instruction.
- Plan review: independent `plan-reviewer` verdict `Ready for approval`,
  Findings none, and `Replanning required: No`.
- Basis: the user requested that Semantic Diff presentation, VS Code host,
  Calendar host, and browser entrypoint placement follow the Flow/Unit List
  policy. Feature Exit is reopened and the prior closure proposal remains
  preserved until this bounded slice is complete.
- Proposed scope: keep `presentation/semantic-diff` as host-neutral output and
  `webview/editor` as the browser entrypoint/feature area; move the Calendar
  host package into `presentation/vscode/semantic-diff/calendar`; remove the
  three obsolete root Semantic Diff host facades; canonicalize imports and
  update the architecture/location guard. Preserve browser bundle names,
  host protocols, DTOs, actions, lifecycle, and generic Flow/Table host
  infrastructure.
- Approved paths (exact Slice 9 plan scope):
  - `src/presentation/vscode/webview/scheduleImpactCalendar/scheduleImpactCalendarJson.ts`
  - `src/presentation/vscode/webview/scheduleImpactCalendar/scheduleImpactCalendarPanel.ts`
  - `src/presentation/vscode/webview/scheduleImpactCalendar/scheduleImpactCalendarPanelRuntime.ts`
  - `src/presentation/vscode/webview/scheduleImpactCalendar/scheduleImpactCalendarSessionRegistry.ts`
...
- Status: Implementation complete on 2026-09-19; focused plan commit
  `65f123ee` is complete and independent implementation review returned
  `Ready` with no Findings. Automatic Completion Approval is recorded below;
  focused completion commit `bb8d7305` is complete.
- Changed paths: five Calendar host modules moved into the canonical
  `src/presentation/vscode/semantic-diff/calendar/` category; three root
  Semantic Diff forwarding facades removed; listed production/test imports
  canonicalized; and `architectureDependencyRules.test.ts` extended for
  category placement, stale-path/facade absence, host-neutral output imports,
  and thin browser entrypoints. `docs/specs/roadmap.md` is excluded from the
  implementation diff.
- Acceptance evidence: Calendar host files resolve only from the new
  `semantic-diff/calendar` package, the old generic Calendar folder and root
  facades are absent, affected consumers resolve to `panel/`, `flow/`, and
  `calendar/` owners, Semantic Diff output remains host-neutral, and webpack
  entry/output names remain unchanged. Runtime contracts, protocols, DTOs,
  sessions, transport, CSP, and Flow/Table infrastructure are unchanged. The
  host-neutral guard uses TypeScript symbols to reject actual DOM-library
  references while permitting local identifiers such as `document`.
- Validation: `rtk pnpm run test:compile` passed; the desktop
  extension-host suite exited 0; the architecture suite passed 27 tests; the
  Calendar bridge/session/transport focused suites passed 14 tests; the web
  test bundle compiled and extension-host smoke passed `WEB-7` through
  `WEB-10`; production, desktop-development, and web-development builds
  compiled successfully;
  full qlty completed with `qlty check: No issues`; Markdown lint passed for
  37 files with 0 errors; and `git diff --check` passed.
- Compatibility/readiness: the relocation preserves VS Code `^1.75.0`, the
  browser-safe thin entries, output bundle names, and host-neutral JSON/report/
  Markdown modules. Existing macOS codesign and webpack-size warnings remain
  documented observations.

### Slice 10 Replan Human Approval

- Status: Approved for the exact Semantic Diff Presentation-report and VS Code
  report package organization below.
- Approved at: 2026-09-19; approved in the current conversation under the
  user's standing automatic no-findings slice approval instruction.
- Plan review: independent `plan-reviewer` verdict `Ready for approval`,
  Findings none, and `Replanning required: No`.
- Basis: the user requested that `src/presentation/semantic-diff` follow the
  Flow/Unit List responsibility structure. Feature Exit is reopened and the
  prior closure proposal and roadmap delta remain preserved and excluded.
- Approved scope: move pure report formatting/projection/localization and
  output-document aggregation into `src/presentation/semantic-diff/report/`;
  keep VS Code output-mode selection and report/document adapters under
  `src/presentation/vscode/semantic-diff/report/`; delete the two obsolete
  `pickSemanticDiffOutputMode.ts`/`presentSemanticDiffOutput.ts` facades; and
  include the unused re-export deletion in
  `src/presentation/webview/editor/semanticDiffExplorer.tsx` as a future
  implementation path. Preserve report content, JSON version 1, mode ordering,
  host contracts, and bundle names. The current replan changes only TASKS and
  TRACEABILITY; it does not edit the browser entrypoint.
- Approved paths (exact Slice 10 plan scope):
  ...
  - `src/presentation/semantic-diff/renderSemanticDiffAuditMarkdown.ts` (moved)
  - `src/presentation/semantic-diff/renderSemanticDiffMarkdown.ts` (moved)
  - `src/presentation/semantic-diff/renderSemanticDiffSummaryMarkdown.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffJson.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffJsonOrdering.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffJsonProjection.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffJsonValidation.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffMarkdownLocalization.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffMarkdownTypes.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffOutput.ts` (moved)
  - `src/presentation/semantic-diff/semanticDiffReportText.ts` (moved)
  - `src/presentation/semantic-diff/serializeSemanticDiffJson.ts` (moved)
...
- Status: Deferred while Slice 16 completes its independent plan, approval,
  implementation, review, and completion gates; explicit human Closure
  Approval follows renewed independent Feature Exit review
- Approved at: none
- Approved scope: none
- Approved paths: none
...
- Dependencies: Slice 3 completion commit `ffb92f1e` plus format correction
  ...
  human approval still batched after all eight slices.
...
- Implementation review: independent `implementation-reviewer` returned
  ...
  led to the completed Slice 6 package relocation.

### Slice 6: Co-locate Calendar Host And Explorer Browser Packages

- Status: Complete on 2026-09-15 after Slice 5 completion commit `f47edeb0`;
  independent plan and implementation reviews are `Ready` with no Findings,
  automatic no-findings Completion Approval is recorded, and focused completion
  commit `c36ee1cf` is complete. Feature Exit follows this slice.
...
- Dependencies: Slice 5 completion commit `f47edeb0` and its approved editor
  ...
  implementation review; final human approval remains batched after all eight
  slices.
...
- Status: Complete on 2026-09-15; independent `implementation-reviewer`
  review is `Ready` with no Findings, automatic no-findings Completion Approval
  is recorded, and focused completion commit `c36ee1cf` is complete. No Slice 6
  behavior or contract changes were introduced.
...
- Unresolved risks: no known implementation, product, scope, or ownership risk
  remains. Existing macOS codesign, web stream-cleanup, webpack-size, and
...
- Trigger: production Explorer rendering still calls the bespoke
  ...
  language from `MyAppContextProvider`. The Explorer host currently rejects
  the provider's common `resource` request before the closed semantic request
  validator, so adopting the provider requires a small shared-resource
  dispatch at that boundary.
- Scope: replace Explorer-specific theme detection with the canonical
  `MyAppContextProvider`; derive Explorer display language and MUI light/dark
  mode from its validated `isDarkMode`/`lang` resource; keep one production
  `ThemeProvider`, `CssBaseline`, and `semanticDiffExplorerGlobalStyles`
  boundary; and preserve the direct `SemanticDiffExplorerView` test seam as a
  pure explicitly injected theme/language view with no environment detection.
  Accept the common viewer `resource` request before semantic request
  validation and reply through the existing `postResourceMessage` and
  `createViewerResourceStateMessage` helper, using the same `vscode` theme and
  language source as Flow/Table. The custom Explorer request union, session
  IDs, action validation, closed transport, calendar callback, and host
  lifecycle remain unchanged.
- User / Domain Value: Explorer receives the actual VS Code theme and locale
  through the same resource contract as the other webviews, so its MUI palette,
  global styles, labels, and status surface render consistently in desktop and
  web extension hosts.
- Cohesive Change Group: shared context/provider wiring, Explorer MUI theme
  ownership, common resource request/response dispatch, and focused context,
  DOM, host-request, and desktop/web regression coverage. No schedule facts,
  hierarchy, actions, or public semantic-diff protocol changes.
- Planned paths and approval scope:
  - Update `src/presentation/webview/editor/MyContexts.tsx` to accept an
    optional `scrollType` prop while preserving the existing `table` default;
    `SemanticDiffExplorerApp.tsx` uses the provider with `window`, matching its
    full-page viewer surface. Update
    `src/presentation/webview/editor/semanticDiffExplorer/SemanticDiffExplorerApp.tsx`
    to wrap the inner app with `MyAppContextProvider`, consume
    `useMyAppContext().isDarkMode`/`lang`, and own one production theme shell
    for loading and loaded states. Remove its bespoke
    `semanticDiffExplorerThemeMode` import and type dependency; keep a local
    presentation mode type or shared MUI type only as needed by the explicit
    View/Contents seam.
  - Update
    `src/presentation/webview/editor/semanticDiffExplorer/SemanticDiffExplorerContents.tsx`
    and `semanticDiffExplorerView.tsx` only to preserve the explicit
    `themeMode` test injection and pass context-derived language/mode from the
    production App. Delete
    `src/presentation/webview/editor/semanticDiffExplorer/semanticDiffExplorerThemeMode.ts`;
    no DOM attribute/class, `matchMedia`, or `MutationObserver` fallback may
    remain in the Explorer presentation package.
  - Extend
    `src/presentation/vscode/semantic-diff/panel/semanticDiffExplorerPanelRequests.ts`
    and its request options/installation in
    `semanticDiffExplorerPanelInstall.ts` (and the narrow type path in
    `semanticDiffExplorerPanelTypes.ts` if required) to parse the existing
    `viewerRequestMessages.parseViewerRequest` resource event before custom
    semantic validation, then call the existing shared
    `src/presentation/vscode/webview/messageHandlers.ts::postResourceMessage`
    for the same panel. Invalid common requests remain fail-closed and custom
    semantic validation/result envelopes remain unchanged. Do not add a new
    Explorer resource schema or duplicate theme/language detection.
  - Replace/delete
    `src/test/suite/semanticDiffExplorerThemeMode.test.ts` with a shared-context
    integration test at the same path or a clearly named
    `semanticDiffExplorerThemeContext.test.tsx`; update
    `semanticDiffExplorerDom.test.tsx` and
    `semanticDiffExplorerComponents.test.tsx` to provide validated resource
    fixtures, assert MUI palette/global-style/theme-mode output and language
    labels, and preserve loading/failure, callback, focus, stable-ID, and
    direct-view seam coverage. Update
    `src/test/suite/semanticDiffExplorerPanel.test.ts` to cover valid common
    resource dispatch before semantic validation, malformed resource safety,
    unchanged semantic request IDs/envelopes, and disposal/reopen behavior.
    Extend existing shared `MyContexts`/resource coverage in
    `viewerHostMessages.test.ts`, `viewerEventBridge.test.ts`, or the nearest
    existing context integration suite rather than introducing a duplicate
    resource contract. Include both desktop and web smoke/host checks.
- Acceptance: Explorer production entry waits for the validated common
  resource state like Flow/Table, then renders one MUI theme using the VS Code
  `isDarkMode` and `lang`; light/dark palette and localized labels are correct;
  no bespoke theme listener/module remains; direct View tests still render with
  explicit injected mode; valid common requests are answered through the
  shared response helper before custom validation; malformed requests fail
  closed; custom semantic messages/actions/session epochs and result envelopes
  behave exactly as before; and desktop/web CSP, bundle names, accessibility,
  focus recovery, calendar actions, and lifecycle cleanup remain intact.
- Validation: run the focused Explorer context/theme, DOM, component, panel,
  shared resource, and event-bridge suites plus existing Explorer/calendar
  regressions; `rtk pnpm run test:compile`; production build; desktop and web
  development builds; desktop and web extension-host/web-smoke checks; the
  architecture/dependency guard for browser-safe imports and removal of the
  bespoke theme module; `rtk pnpm run qlty`; `rtk pnpm run lint:md`; and
  `git diff --check`. The DOM matrix must cover light/dark resource fixtures,
  localized EN/JA/fallback labels, loading until resource arrival, and direct
  test-seam injection.
- Production Readiness: reuse the existing validated viewer resource DTO and
  `postResourceMessage` source of truth; preserve `scrollType` semantics and
  the current provider cleanup; keep Explorer custom transport/session
  validators isolated from the generic side channel; preserve browser-safe
  webview imports, nonce/CSP and output filenames, VS Code compatibility,
  accessibility, privacy, and telemetry. This is an internal theme/context
  correction, so no README/CHANGELOG update is expected unless implementation
  changes observable documented behavior.
- Approval Boundary: only the shared provider prop, Explorer App/Contents/View
  theme wiring, deletion of the bespoke theme module and its test, the narrow
  generic-resource dispatch/request-option plumbing, and the listed focused
  regression/architecture tests. Schedule semantics, comparison/artifact
  DTOs, public Explorer message/action unions, calendar transport, workflow,
  host lifecycle, shared resource schema, and unrelated viewer behavior are
  outside this slice.
- Dependencies: completion-committed Slice 6 `c36ee1cf`, existing
  `MyAppContextProvider`/viewer resource parser and response helper, and the
  current Explorer editor package. Slice 7 follows Slice 6 and must receive
  independent plan review, Human Approval, approval-committer plan gate,
  implementation review, and Completion Approval. The user's automatic
  no-findings approval applies after a clean independent implementation
  review; final human approval remains batched after all eight slices.
- Risks: Explorer's custom host validator must recognize the generic resource
  side channel without weakening session/action validation; a missing response
  can leave the provider intentionally waiting; language from the common
  resource must remain presentation-only while custom session data remains
  intact; duplicate production ThemeProviders or a residual listener could
  mask the fix; and desktop/web host differences could regress startup. Focused
  resource, context, disposal, DOM palette, compile, and host tests cover these
  risks.
- Out of Scope: any new resource/message schema, public semantic-diff union,
  action/session/transport behavior, schedule meaning, hierarchy/tree/data
  changes, calendar UI, parser/application/domain changes, shared Flow/Table
  redesign, dependency changes, CSP/output-name changes, persistence,
  telemetry, and unrelated documentation.

### Slice 7 Planning Evidence

- Replanning trigger and impact are recorded above. Investigation confirmed
  that `MyAppContextProvider` is the canonical Flow/Table mechanism and that
  Explorer's current custom host request validator rejects its valid resource
  request. The plan therefore reopens Feature Exit without changing the
  completed Slices 1–6 or their evidence.
- No normative `SPECS.md` amendment is required: this is a presentation and
  shared-host-adapter correction within the existing Explorer/theme
...
- Dependencies: completion-committed Slice 7 `8c555139`, existing
  ...
  instruction, and focused completion commit `fe042fb0` is complete. Final
  human approval remains batched after all eight slices.
...
### Slice 9: Align Semantic Diff Host And Webview Package Placement

- Status: Implementation complete under focused plan commit `65f123ee`.
  Independent `implementation-reviewer` returned `Ready` with no Findings.
  automatic Completion Approval is recorded below; focused completion commit
  `bb8d7305` is complete.
- Trigger: browser entries already follow the common thin
  `webview/editor/<viewer>.tsx` pattern, but Calendar's feature-specific host
  package remains under generic document-viewer infrastructure at
  `presentation/vscode/webview/scheduleImpactCalendar`, and Semantic Diff host
  roots retain three forwarding facades beside the `panel`, `flow`, `report`,
  and `source` categories. The host-neutral output modules under
  `presentation/semantic-diff` have a different responsibility and should
  remain separate.
- Placement policy: `src/presentation/webview/editor/*.tsx` is limited to
  thin browser bundle bootstraps that call `bootstrapViewer`; browser feature
  modules live under the adjacent `editor/<feature>/` package and remain free
  of `vscode` and Node imports. `src/presentation/vscode/webview/` remains the
  shared document-viewer host infrastructure used by Flow/Table
  `ViewerFactory`/mediator/store/mounting and shared message helpers. Semantic
  Diff-specific VS Code adapters live under
  `src/presentation/vscode/semantic-diff/<category>/`, including a new
  `calendar/` category for the host-private Calendar child panel. Pure output,
  report, JSON, and Markdown presentation modules remain under
  `src/presentation/semantic-diff/` and must not acquire VS Code, React/MUI,
  browser, or Node dependencies.
- Scope and exact moves:
  - Move these five host-only files from
    `src/presentation/vscode/webview/scheduleImpactCalendar/` to
    `src/presentation/vscode/semantic-diff/calendar/`, preserving file names,
    exports, session/transport behavior, CSP/bundle construction, and the
    generic `messageHandlers.ts`/viewer-resource helper as a shared dependency:
    `scheduleImpactCalendarJson.ts`, `scheduleImpactCalendarPanel.ts`,
    `scheduleImpactCalendarPanelRuntime.ts`,
    `scheduleImpactCalendarSessionRegistry.ts`, and
    `scheduleImpactCalendarTransport.ts`.
  - Remove the three root forwarding facades
    `src/presentation/vscode/semantic-diff/semanticDiffExplorerPanel.ts`,
    `semanticDiffExplorerRegistry.ts`, and `semanticDiffExplorerFlow.ts`.
    Update internal consumers to import the canonical `panel/`, `flow/`,
    `report/`, `source/`, and new `calendar/` modules. Repository search shows
    no external package consumer; compile and source-location guards must prove
    that no stale facade import remains.
  - Keep `src/presentation/semantic-diff/` in place as the host-neutral
    semantic-output package. Keep the browser entries
    `src/presentation/webview/editor/scheduleImpactCalendar.tsx` and
    `semanticDiffExplorer.tsx` in place beside `flowViewer.tsx` and
    `tableViewer.tsx`; preserve all four webpack entry names and output bundle
    names. No webpack entry rewrite is expected, but the architecture guard
    must assert the canonical entry paths and thin bootstrap shape.
- Planned production/import paths: update
  `src/bootstrap/extension/semanticDiffWiring.ts`,
  `src/bootstrap/extension/semanticDiffFlowViewerBridge.ts`,
  `src/presentation/vscode/commands/semanticDiffCommand.ts`,
  `semanticDiffCommandExplorerWorkflow.ts`,
  ...
  `semanticDiffExplorerPanelTypes.ts`, and
  `src/presentation/webview/editor/scheduleImpactCalendar/scheduleImpactCalendarBridge.ts`.
  Their imports must resolve to the new canonical category paths without
  changing factories, action IDs, callbacks, session IDs, or message unions.
- Planned test and guard paths: update the import fixtures in
  `src/test/suite/semanticDiffCommand.test.ts`,
  `semanticDiffCommandScheduleImpact.test.ts`,
  `semanticDiffExplorerFlow.test.ts`,
  `semanticDiffExplorerPanel.test.ts`,
  `semanticDiffExplorerRegistry.test.ts`,
  `semanticDiffExplorerSourceAction.test.ts`,
  `semanticDiffWiring.test.ts`,
  `scheduleImpactCalendarBridge.test.ts`,
  `scheduleImpactCalendarPanelRuntime.test.ts`,
  `scheduleImpactCalendarSession.test.ts`,
  `scheduleImpactCalendarTransport.test.ts`,
  `semanticDiffExplorerScheduleImpact.test.ts`, and `webSmoke.ts`.
  Extend `src/test/suite/architectureDependencyRules.test.ts` to assert the
  new Calendar host category, absence of the generic old Calendar folder and
  root facades, canonical category imports, host-neutral output dependency
  restrictions, browser entry/bootstrap shape, no browser Node/VS Code imports,
  unchanged webpack entry/output names, and no stale paths. Existing behavior
  suites remain the semantic and lifecycle regression evidence.
- Acceptance: the five Calendar host modules exist only under
  `presentation/vscode/semantic-diff/calendar`; generic `vscode/webview`
  contains only shared viewer infrastructure and helpers; the three Semantic
  Diff root facades are gone and all imports use category owners; pure
  `presentation/semantic-diff` output remains host-neutral; Calendar and
  Explorer browser entries remain thin `bootstrapViewer` entrypoints with
  unchanged `scheduleImpactCalendar`/`semanticDiffExplorer` bundle names; and
  Flow/Table ViewerFactory behavior is untouched. Runtime behavior, public
  contracts, session/action/message protocols, schedule facts, CSP, and
  desktop/web output remain unchanged.
- Validation: run the focused architecture/location/import guard and all
  affected Semantic Diff, Calendar, bridge/session/transport, web-smoke, and
  browser component suites; run `rtk pnpm run test:compile`, production and
  desktop/web development builds, desktop/web extension-host checks, qlty,
  Markdown lint, and `git diff --check`. Include a source inventory check that
  proves the host-neutral output package does not import `vscode`, React/MUI,
  browser globals, or Node built-ins, and that browser entries do not import
  host modules.
- Production Readiness: preserve VS Code `^1.75.0`, browser-safe shared code,
  JP1/AJS3 behavior, JSON/report/output contracts, action and session identity,
  CSP/nonces, bundle filenames, telemetry, and Flow/Table document-viewer
  lifecycle. No README/CHANGELOG, roadmap, SPECS, dependency, or public API
  change is planned. The old Calendar path and root facades are internal and
  have no external consumers according to the repository import inventory.
- Approval Boundary: only the listed Calendar host move, Semantic Diff facade
  removal, canonical import updates, architecture/location guard updates, and
  affected tests. Do not move generic ViewerFactory/WebviewMediator/
  WebviewStore infrastructure, browser feature modules, host-neutral semantic
  output modules, webpack bundle names, schedule/application/domain code,
  protocols, DTOs, actions, lifecycle, or roadmap/closure documentation.
- Dependencies: completion-committed Slice 8 `fe042fb0`, the existing
  `presentation/vscode/semantic-diff` category packages, current Calendar
  browser package and thin entries, shared viewer host infrastructure, and
  architecture dependency rules. Slice 9 requires independent plan review,
  Human Approval, approval-committer plan gate, implementation review, and
  Completion Approval. The user's automatic no-findings approval applies only
  after a clean independent implementation review; final human approval stays
  batched after all nine slices.
- Risks: relative import changes may cross the host/browser boundary, deleting
  facades may miss a non-source consumer, moving the Calendar host may alter
  shared-helper resolution, and an over-broad location guard could reject
  legitimate host-neutral output imports. Repository-wide import inventory,
  compile, architecture/location assertions, bundle checks, and desktop/web
  host tests cover these risks.
- Out of Scope: behavior or meaning changes, protocol/DTO/action changes,
  generic ViewerFactory lifecycle changes, browser component refactors,
  `presentation/semantic-diff` semantic-output redesign, webpack output/name
  changes, dependency/configuration changes, documentation/roadmap changes,
  telemetry, persistence, and Feature Exit closure.

### Slice 9 Planning Evidence

- Replanning trigger and the completed Slice 1–8 state are recorded above.
  Feature Exit is reopened only for this bounded placement-policy slice; the
  prior closure proposal and roadmap delta remain preserved and unapproved.
- No normative `SPECS.md` amendment is required at planning time: the slice
  changes source placement and ownership clarity while preserving the feature
  purpose, public behavior, and architecture boundaries. If implementation
  discovers an external consumer, a required public export, or a boundary
  change beyond these paths, stop and return to Main for Replanning.
- Plan review and approval: independent `plan-reviewer` returned `Ready for
approval` with no Findings and `Replanning required: No`. Human Plan
  Approval was recorded on 2026-09-19 under the user's automatic no-findings
  instruction for the exact paths above. The next route is
...
- Status: Approved
- Approved at: 2026-09-19; approved in the current conversation under the
  user's standing automatic no-findings Completion Approval instruction.
- Basis: independent `implementation-reviewer` final verdict `Ready`;
Findings none. The completed diff matches the reviewed Slice 9 placement,
import, facade-removal, and architecture/location guard scope.
<!-- markdownlint-disable MD013 -->
- Approved paths (exact completed diff):
  - `docs/specs/features/schedule-impact-calendar/TASKS.md`
  - `docs/specs/features/schedule-impact-calendar/TRACEABILITY.md`
...
- Status: Implementation complete on 2026-09-19 under focused plan commit
  ...
  Independent `implementation-reviewer` returned `Ready` with no Findings;
  automatic Completion Approval is recorded below and focused completion commit
  `0a5cdd31` is complete.
- Trigger and policy: the fourteen files under
  ...
  Quick Pick adapter. The durable architecture assigns localization, report
  projections, and mode selection to Presentation. The canonical policy puts
  host-neutral report work under the Presentation report package, VS Code
  chooser/document adapters under the VS Code Semantic Diff report package,
  and browser rendering only under `presentation/webview/editor/<feature>/`.
- Presentation report moves (host-neutral formatting, projection, localization,
  and document dispatch): move these files to
  `src/presentation/semantic-diff/report/`, preserving exports and output
  behavior:
  ...
  - `semanticDiffOutput.ts` (retain pure document dispatch/types and extract
    the picker metadata/operation described below)
  - `semanticDiffReportText.ts`
  - `serializeSemanticDiffJson.ts`
    The Presentation report package remains free of VS Code, React/MUI, browser
    globals, and Node built-ins; the existing static `@resource/i18n/message`
    table remains the localization input and is not replaced by host APIs.
- VS Code report-owned extraction: add
  `src/presentation/vscode/semantic-diff/report/semanticDiffOutputModePicker.ts`
  for `SemanticDiffOutputModeItem`, mode-item ordering, the Quick Pick callback
  type, and `pickSemanticDiffOutputMode`. It owns the VS Code chooser boundary;
  `semanticDiffReportDocument.ts` and Explorer report actions remain in the
  existing VS Code report package and consume Presentation report outputs plus
  application DTOs.
- Facade cleanup and browser entry: delete
  `src/presentation/semantic-diff/pickSemanticDiffOutputMode.ts` and
  `src/presentation/semantic-diff/presentSemanticDiffOutput.ts`; remove the
  now-empty root package after the report move. Update
  `src/presentation/webview/editor/semanticDiffExplorer.tsx` only to preserve
  its `bootstrapViewer(SemanticDiffExplorerApp)` entry and remove the unused
  named/default `SemanticDiffExplorerApp` re-exports in the approved
  implementation diff. Keep webpack entry/output names unchanged; the current
  replan diff contains no runtime change.
- Production import consumers: update
  `src/bootstrap/extension/semanticDiffWiring.ts`,
  `src/presentation/vscode/commands/semanticDiffCommand.ts`,
  `semanticDiffCommandSelection.ts`, `semanticDiffCommandSteps.ts`,
  `semanticDiffCommandWorkflowExecution.ts`,
  `semanticDiffCommandWorkflowSelection.ts`,
  `src/presentation/vscode/semantic-diff/panel/semanticDiffExplorerPanelTypes.ts`,
  `src/presentation/vscode/semantic-diff/report/semanticDiffExplorerReportActionRunner.ts`,
  `semanticDiffExplorerReportActionTypes.ts`, and
  `semanticDiffReportDocument.ts`. Imports must resolve to the Presentation
  report transformations or the VS Code picker without changing dependency direction,
  report actions, document lifecycle, mode order, or public types.
- Tests and guards: update
  `src/test/suite/architectureDependencyRules.test.ts`,
  `compareSemanticDiff.test.ts`, `nls.test.ts`,
  `renderSemanticDiffMarkdown.test.ts`, `semanticDiffCommand.test.ts`,
  `semanticDiffConditions.test.ts`, `semanticDiffJson.test.ts`,
  `semanticDiffMarkdownProjections.test.ts`, `semanticDiffOutput.test.ts`,
  `semanticDiffReportDocument.test.ts`, and
  `semanticDiffSampleCoverage.test.ts`; add focused
  `src/test/suite/semanticDiffOutputModePicker.test.ts` coverage for the VS
  Code chooser. The architecture guard must assert that the old package and
  facades are absent, Presentation report files contain no host/UI/browser/Node
  imports, VS Code report adapters consume Presentation report code and
  application DTOs through existing inward dependencies,
  and `semanticDiffExplorer.tsx` has no unused App re-exports.
- Acceptance: all fourteen old files are accounted for as twelve Presentation
  report moves, one VS Code picker extraction from `semanticDiffOutput.ts`,
  and two facade deletions; no production import references the old package;
  report Markdown/JSON bytes, locale fallback, mode ordering, picker
  cancellation, document open/save/copy lifecycle, Explorer report actions,
  and browser bootstrap behavior remain unchanged.
- Validation: run the focused report/JSON/Markdown/picker/document/action suites,
  affected command and Explorer suites, architecture/location/dependency
  guards, `rtk pnpm run test:compile`, production and desktop/web development
  builds, desktop/web extension-host checks, qlty, Markdown lint, and
  `git diff --check`. Verify webpack entry/output names and that the browser
  entry remains `bootstrapViewer`-only.
...
- Risks: the report subpackage must remain Presentation-owned under the durable
  ...
  import inventory, compile, report/document/action tests, and architecture
  checks cover these risks.
- Out of Scope: report wording/JSON schema/mode semantics, schedule meaning,
  Explorer or Calendar protocols/actions/session behavior, browser UI changes
  beyond the unused re-export deletion, webpack configuration/output names,
  dependency changes, SPECS, README/CHANGELOG, roadmap, telemetry, and Feature
  Exit closure.

### Slice 10 Planning Evidence

- Replanning trigger and the completed Slice 1–9 state are recorded above.
  Feature Exit is reopened only for this bounded report-package placement
  slice; the prior closure proposal and roadmap delta remain preserved and
  excluded.
- Finding addressed: the earlier application/report destination conflicted with
  `docs/specs/architecture.md` Presentation ownership of localization, report
  projections, and mode selection. The revised destination is
  `src/presentation/semantic-diff/report/`; no durable architecture document
  is changed. The browser re-export deletion remains a future approved path;
  the current replan diff contains no runtime entrypoint edit.
- No normative `SPECS.md` amendment is required: this slice clarifies package
  ownership and import direction while preserving public report, JSON,
...
### Slice 11: Shared Readable Result Presentation
...
- Replanning trigger: the user reported that Semantic Diff Explorer and
  ...
  MUI and the shared viewer theme/context, but section, card, label/value,
  status, and empty-state markup is duplicated or represented as `·`-joined
  prose in each view.
- Scope: add browser-safe shared result primitives under
  `src/presentation/webview/editor/shared/result/`: `ResultSection.tsx`,
  `ResultCard.tsx`, `ResultKeyValueList.tsx`, `ResultStatusChip.tsx`, and
  `ResultEmptyState.tsx`. Primitives accept generic labels and `ReactNode`
  values, use the existing MUI theme and `viewerThemeStyles`, and do not
  perform domain formatting. Keep localized labels, outcome mapping, detail
  ordering, and malformed-data wording in the owning view adapters.
- Explorer adoption: update
  `src/presentation/webview/editor/semanticDiffExplorer/SemanticDiffExplorerContents.tsx`,
  `SummaryCards.tsx`, `ExplorerTreePanel.tsx`, and
  `semanticDiffExplorerTree.tsx` to use the shared section/card/key-value/
  status primitives for summary, status, row metadata, and detail values.
  Preserve the Header/actions, tree navigation, selection callbacks, live
  announcements, focus recovery, and existing `SemanticDiffOutputContext`.
- Calendar adoption: update
  `src/presentation/webview/editor/scheduleImpactCalendar/ScheduleImpactCalendarContents.tsx`,
  `ScheduleImpactCalendarHeader.tsx`,
  `ScheduleImpactCalendarSections.tsx`, and
  `ScheduleImpactCalendarTimeline.tsx` to use the same primitives for the
  title/summary hierarchy, root/no-run/candidate/issue/legend sections,
  timeline entries, run details, status outcomes, and empty states. Keep
  `ScheduleImpactCalendarBoundedList.tsx`'s virtualization and keyboard
  contract unchanged; shared cards must remain safe for its cloned focusable
  children.
- Information hierarchy: both views expose title/header, summary/status,
  metadata/context, and detail sections through semantic `section` headings,
  outlined MUI cards, `dl` label/value rows, and generic status chips. Long
  paths, IDs, and detail values wrap within cards. Empty, unavailable, and
  malformed-data states remain the existing localized view states and use the
  generic empty primitive without inferring business outcomes.
- Visual and accessibility contract: use MUI theme palette, existing
  `Stack` spacing and typography scale, responsive column-to-row layouts,
  existing focus/forced-colors styles, and the shared minimum target size.
  Preserve heading levels, region/list/tree semantics, `aria-labelledby`,
  `role=status` live/empty announcements, screen-reader labels, keyboard
  navigation, reduced-motion behavior, and desktop/web rendering. Do not add
  a view-specific theme listener or formatter.
- User / Domain Value: the same schedule and semantic-diff facts become
  scannable by status, metadata, and detail without changing what the facts
  mean or which actions are available.
- Cohesive Change Group: the five shared primitives plus both view adapters,
  their focused component/accessibility tests, and one architecture guard
  update form one independently reviewable presentation slice. The shared
  primitives remain presentation-only and consume no domain, application,
  infrastructure, bootstrap, VS Code, parser, or host-session modules.
- Acceptance:
  - Explorer and Calendar both render the shared section/card/key-value,
    status, and empty primitives; no duplicate generic result primitives are
    introduced under either view directory.
  - Summary, status, metadata, and detail are visibly separated with stable
    localized labels; long identifiers wrap and empty/unavailable/malformed
    states remain readable and truthful.
  - Responsive reflow, theme palette, high-contrast/focus styling,
    heading/region/list/tree semantics, live announcements, keyboard focus,
    and screen-reader labels remain valid in both views.
  - Large Explorer trees and Calendar timelines retain existing Virtuoso/
    bounded-list thresholds, overscan, focus recovery, DOM bounds, ordering,
    and action callbacks.
  - `SemanticDiffOutputContext`, Calendar DTOs, schedule meaning, filters,
    report output, public action/message unions, bridges, sessions, host
    lifecycle, CSP, and desktop/web bundle names are byte/behavior compatible.
  - `CHANGELOG.md` contains one concise `Unreleased` user-facing entry:
    ...
    presentation." The entry describes the visible result-display improvement
    without exposing internal component names or changing product meaning.
- Validation: add
  `src/test/suite/sharedResultPresentation.test.tsx` for primitive semantics,
  long-value wrapping, status tones, empty-state roles, and focusable card
  labels. Extend `semanticDiffExplorerComponents.test.tsx`,
  `semanticDiffExplorerDom.test.tsx`,
  `scheduleImpactCalendarComponents.test.tsx`,
  `scheduleImpactCalendarView.test.tsx`, and
  `scheduleImpactCalendarAccessibility.test.tsx` for hierarchy, shared
  primitive usage, localization, empty/malformed/unavailable states,
  responsive values, keyboard/screen-reader behavior, and large fixtures.
  Reuse `semanticDiffExplorerThemeContext.test.tsx` and
  `scheduleImpactCalendarThemeContext.test.tsx` to prove the existing common
  theme/context remains the source of palette and locale. Update
  `architectureDependencyRules.test.ts` to guard the shared package's
  presentation-only imports and both consumers' canonical imports. Run the
  focused suites, `rtk pnpm run test:compile`, production/desktop/web builds,
  desktop/web host checks, `rtk pnpm run qlty`, Markdown lint,
  `CHANGELOG.md`/feature Markdown lint, and `git diff --check`; verify the exact
  Unreleased entry and absence of unrelated changelog edits.
- Production Readiness: malformed or missing display values must render the
  existing localized fallback rather than throwing or inventing facts; long
  values must not overflow or create unbounded layout width; large lists must
  remain virtualized. Shared browser code must stay free of Node/VS Code
  imports, and common theme/resource behavior must remain unchanged on desktop
  and web. Because this is user-visible readability behavior, add the concise
  `Unreleased` entry above to `CHANGELOG.md`; no README, SPECS, roadmap, or
  durable architecture change is planned.
- Approval Boundary: the exact approved paths are recorded in the Slice 11
  Plan Review And Human Approval section below: shared result files, listed
  Explorer and Calendar presentation consumers, focused tests, architecture
  guard, `CHANGELOG.md`, and TASKS/TRACEABILITY. No other path is approved.
  The CHANGELOG edit is implementation work within the approved slice and must
  not be made before the focused plan commit.
...
- Risks: generic primitives could absorb semantic formatting or alter row
  ...
  size or reduce contrast; retain bounded lists and assert theme/forced-colors
  styles. Any DTO, protocol, lifecycle, or theme-mechanism change requires a
  new replan and approval boundary.
- Out of Scope: schedule evaluation, semantic-diff projection/report/JSON
  behavior, Calendar or Explorer DTOs, actions/messages, bridges, transport,
  sessions, host adapters, parser/application changes, dependency upgrades,
  webpack/CSP changes, Flow/Unit List behavior, durable architecture/SPECS
  changes, roadmap closure, Feature Exit, and implementation/commit/approval.

### Slice 11 Plan Review And Human Approval

- Plan-reviewer result: `Ready for approval`; Findings none; `Replanning
required: No`.
- Human Plan Approval: Approved on 2026-09-20 in the current conversation
  under the user's standing automatic no-findings slice approval instruction.
- Approved boundary: add the shared browser-safe result primitives, adopt them
in the listed Explorer and Calendar presentation components, add the focused
shared/view accessibility/theme and architecture coverage, and add exactly
one concise user-facing `Unreleased` entry to `CHANGELOG.md`. Preserve all
facts, labels, DTOs, filters, actions, messages, sessions, common theme/
resource handling, virtualization, and desktop/web behavior.
<!-- markdownlint-disable MD013 -->
...
- Status: Plan review returned `Ready for approval` with no Findings and
  ...
  commit `b25e38d5` is complete. Implementation is complete under the approved
  paths; final implementation review returned `Ready` with no Findings,
  automatic Completion Approval is recorded below, and focused completion
  commit `2fb18daa` is complete.
...
- Plan gate status: complete; focused Slice 12 plan commit `b25e38d5`.
  ...
  complete. Next stage: independent Feature Exit review and final batch human
  Closure Approval.

### Slice 12 Implementation Evidence

- Status: Implementation complete under focused plan commit `b25e38d5`.
  Final `implementation-reviewer` verdict: `Ready`; Findings none. Automatic
  Completion Approval is recorded below under the user's standing no-findings
  instruction. Focused completion commit `2fb18daa` is complete.
- Acceptance result: the shared localized range formatter and comparison shell
  are adopted by the Calendar and Explorer consumers in the approved scope;
  before remains first/left and after second/right, narrow layouts preserve
  before→after order, and the existing facts, DTOs, half-open period semantics,
  keyboard/focus behavior, virtualization, and theme/resource mechanism remain
  unchanged.
- Changed paths are listed exactly in the Slice 12 Completion Approval below;
  no unapproved component, runtime protocol, roadmap, or unrelated changelog
  path is included.
...
### Slice 13: Per-Row Result Alignment And Targeted Smell Cleanup
...
- Replanning trigger: `ResultKeyValueList` can visually pack adjacent dense
  ...
  primitive is used by Calendar sections/timeline and Explorer summary/tree
  details. The qlty inventory also shows five findings across three modules
  introduced after the Slice 10 baseline: `ResultKeyValueList.tsx`,
  `SemanticDiffExplorerContents.tsx`, and
  `semanticDiffExplorerTree.tsx`.
- Scope: change `ResultKeyValueList.tsx` so every item is an independent grid
  row spanning the label/value columns. Keep the semantic `<dl>`, `<dt>`, and
  `<dd>` structure, source order, optional IDs, long-value wrapping, and
  narrow-layout reflow. Retain the `dense` prop only as a spacing/compactness
  choice; it must never pack multiple items into one visual row. `ResultComparison`
  remains the shared before-first/after-second shell and receives regression
  coverage for key/value lists in both side slots without changing its public
  props or focus behavior.
- Scope: simplify `SemanticDiffExplorerContents.tsx` by extracting its status
  presentation from the main composition function while preserving the common
  resource/theme context, host-announcement effect, filter state, selection,
  actions, and tree focus lifecycle. Move the pure detail-row/comparison
  builders from `semanticDiffExplorerTree.tsx` into a new browser-safe
  `semanticDiffExplorerDetails.ts` module. Split optional/detail and schedule
  value formatting into small helpers so the tree keeps rendering, action, and
  focus responsibilities without the Slice 12 complexity findings. Preserve
  localized labels, range formatting, before/after values, constraints,
  warnings, and existing tree row semantics.
- User / Domain Value: each Calendar and Explorer label/value fact is readable
  as one aligned row, including long paths and nested before/after details;
  users keep the same facts and controls while the targeted new quality
  findings are removed.
- Cohesive Change Group: the shared row primitive, Explorer detail helper
  extraction, Calendar/Explorer DOM assertions, browser-safe architecture
  guard, one concise user-facing changelog entry, and qlty baseline check form
  one independently reviewable presentation slice.
- Acceptance:
  - Each `ResultKeyValueList` item, dense or normal, renders one row with one
    label and one value; no two item containers share a visual row. Labels and
    values remain aligned, long labels/paths wrap, narrow layouts remain
    readable, and `<dl>/<dt>/<dd>` semantics and screen-reader order persist.
  - Calendar root, no-run, issue, candidate, and timeline details, including
    nested source-change references and both `ResultComparison` sides, satisfy
    the one-row invariant. Explorer summary cards and tree detail rows satisfy
    the same invariant, with before then after source order and unchanged
    localized labels, actions, announcements, and focus recovery.
  - The targeted Slice 11/12 findings are absent from the post-change qlty
    smell inventory. Findings already present at the Slice 10 baseline remain
    explicitly inventoried and are not suppressed, ignored, or refactored by
    this slice. No qlty configuration changes are allowed.
  - Calendar/Explorer DTOs, facts, filters, result/report/JSON contracts,
    action/message unions, sessions, transport, theme/resource mechanism,
    virtualization, bundle names, and desktop/web behavior remain unchanged.
  - `CHANGELOG.md` gains one concise `Unreleased` entry for clearer one-row
    result details; unrelated entries remain unchanged.
- Validation: extend `sharedResultPresentation.test.tsx` for dense and normal
  row structure, one-label/one-value alignment, long wrapping, narrow reflow,
  `<dl>/<dt>/<dd>` semantics, and comparison-side source order. Extend
  `scheduleImpactCalendarComponents.test.tsx`,
  `scheduleImpactCalendarView.test.tsx`, and
  `scheduleImpactCalendarAccessibility.test.tsx` for Calendar root/no-run,
  issue, candidate, timeline, one-sided, long-value, and nested-comparison
  rows. Extend `semanticDiffExplorerComponents.test.tsx` and
  `semanticDiffExplorerDom.test.tsx` for summary/tree rows, extracted detail
  formatting, localized values, long values, actions, and focus. Update
  `architectureDependencyRules.test.ts` to include the new browser-safe detail
  module and retain the shared-import guard. Compare qlty output with the
  Slice 10 baseline and run `rtk pnpm run qlty`, `rtk pnpm run test:compile`,
  focused desktop/web UI suites, production/desktop/web builds and host checks,
  Markdown lint, and `git diff --check`.
- Production Readiness: no values are parsed or normalized; malformed,
  missing, one-sided, and large values remain truthful and bounded. The row
  helper and Explorer detail module stay browser-safe and free of Node/VS Code
  imports. Existing baseline qlty findings, macOS codesign, stream-cleanup,
  and bundle-size observations remain recorded as compatibility observations.
- Approval Boundary: the exact approved paths are the shared key/value
...
- Dependencies: Slice 13 depends on completion-committed Slice 12
  ...
  independent review.
- Risks: changing grid placement can affect nested `dl` semantics, dense
  source-change references, comparison side sizing, or small-screen wrapping;
  keep the primitive's public props stable and assert the row structure in
  shared and both view fixtures. Moving Explorer detail builders can disturb
  locale or date-range output; retain the existing helper inputs and assert
  localized/before-after output. A required change to DTOs, protocols,
  virtualization, or shared theme/resource handling requires another replan.
- Out of Scope: schedule calculation, date parsing, DTO/protocol/message
  changes, host/session/transport changes, Calendar/Explorer business facts,
  report/JSON behavior, Flow/Unit List behavior, qlty suppression or config
  changes, unrelated Slice 10-baseline smell cleanup, dependency upgrades,
  roadmap closure, Feature Exit, and implementation/commit/approval.
...
### Slice 14: Remove Internal Identifiers From Calendar Presentation

- Status: Plan review is `Ready` with no Findings and `Replanning required: No`;
  Human Plan Approval is recorded below. Focused Slice 14 plan commit
  `9a0bd721` is complete, implementation is complete under its approved
  paths, and independent implementation review and Completion Approval are
  complete; focused completion commit `52ede211` is complete. The user asked
  whether Calendar `ID` fields are necessary because they appear to be internal
  identifiers and the Flow, Unit List, and Explorer views present names and
  paths instead. This slice records the evidence-based presentation boundary
  before any implementation.
- Evidence and decision: `AjsUnit.id` is assigned from `absolutePath` by the
  parser and Calendar already presents `unitPath`, `unitName`, and root paths.
  Calendar `root.id`, timeline-item `id`, run `id`, candidate/group `id`, issue
  `id`, `identityDecisionId`, and `sourceChangeRef.id` are stable sidecar or
  comparison references documented for matching, validation, sorting, focus,
  DOM attributes, and diagnostics. They are internal data, not user-facing
  AJS facts. `targetId` remains a schedule issue target value and
  `unitName`/`unitPath` remain user-facing source identity. The generic `ID`,
  `Source unit ID`, `Identity decision ID`, and encoded source-change ID rows
  should therefore be removed from rendered Calendar result details while
  preserving all values in DTOs, model state, filters, data attributes,
  keyboard focus, virtualization, validation, and diagnostics.
- Scope: update Calendar Sections and Timeline rendering to omit internal ID
  rows, remove internal IDs from every user-facing card title, comparison
  label, accessible name, `aria-label`, and `aria-live` announcement, and keep
  meaningful path/name/target/occurrence context. Replace Timeline Enter/Space
  `labels.selected(itemId)` announcements with a localized selected message
  carrying path/name/target/occurrence context or a generic localized selected
  message. Replace candidate before/after bounded-list labels that currently
  include `group.id` with localized semantic context. Remove the now-unused
  English/Japanese internal-ID labels. Keep source-change references and
  identity decisions in the sidecar and model; this slice changes only their
  presentation, not their contracts.
- User / Domain Value: Calendar details match the other views' readable
  identity policy, showing the AJS unit name/path and schedule facts without
  exposing implementation keys that users cannot act on.
- Acceptance:
  - No rendered Calendar `dt`/`dd`, card title, comparison label, accessible
    name, `aria-label`, or `aria-live` announcement exposes stable sidecar IDs,
    run/item IDs, candidate/group IDs, issue IDs, `unitId`,
    `identityDecisionId`, or encoded `sourceChangeRef.id`. DOM `data-*` keys,
    focus selectors, model/filter keys, and sidecar DTO values remain unchanged.
  - Root, no-run, candidate, issue, and timeline details retain source names,
    paths, target IDs, rule/occurrence facts, outcomes, scope-transition
    meaning, before/after order, localization, and screen-reader context.
  - English/Japanese localization has no unused internal-ID labels, and all
    keyboard announcements use localized semantic context without internal IDs.
    Duplicate names, one-sided roots, candidate before/after groups, and
    long/malformed details remain distinguishable through user-facing context.
    Existing Flow, Unit List, and Explorer display behavior is unchanged.
  - No DTO, protocol, report/JSON, schedule calculation, session, transport,
    filter, virtualization, or focus behavior changes.
- Planned paths and approval scope:
  - `src/presentation/webview/editor/scheduleImpactCalendar/ScheduleImpactCalendarSections.tsx`
  - `src/presentation/webview/editor/scheduleImpactCalendar/ScheduleImpactCalendarTimeline.tsx`
  - `src/presentation/webview/editor/scheduleImpactCalendar/scheduleImpactCalendarModel.ts`
  ...
  - `docs/specs/features/schedule-impact-calendar/TASKS.md`
  - `docs/specs/features/schedule-impact-calendar/TRACEABILITY.md`
- Validation: add focused render assertions for absent internal-ID labels and
  preserved user-facing names/paths/target IDs, verify accessible names,
  ...
  before/after labels without group IDs. Update model-label tests, run the
  Calendar component/view/accessibility/localization/projection suites,
  `rtk pnpm run test:compile`, desktop/web build and host checks,
  `rtk pnpm run qlty`, and compare `qlty smells --no-snippets` with the Slice
  13/origin-main inventory. Existing Calendar Sections/Timeline/model
  complexity and locale duplication findings are baseline observations; no
  new smell or suppression/configuration change is accepted. Run Markdown lint
  and `git diff --check`.
- Dependencies: Slice 14 depends on completion-committed Slice 13 `b8d9a667`.
  It must complete its own plan review, Human Plan Approval, implementation
  review, Completion Approval, and focused commit before Feature Exit can be
  reconsidered.
- Risks: removing IDs from accessible names can reduce debugging context if
  replacement path/name labels are ambiguous; retain user-facing path/name and
  stable DOM attributes and test duplicate names, one-sided roots, malformed
  details, and long paths. A request to alter DTO fields, ID generation,
  source-change semantics, or other views requires Replanning and a wider
  boundary decision.
- Out of Scope: DTO/schema/protocol changes, ID generation, schedule meaning,
  source-change resolution, report/JSON output, Flow/Unit List/Explorer UI
  changes, host/session/transport/theme changes, virtualization/focus logic,
  qlty suppression/configuration, roadmap closure, Feature Exit, and
...
### Slice 15: Shared MUI Presentation Audit For Explorer And Calendar
...
- Status: Plan review is `Ready` with no Findings and `Replanning required: No`.
  ...
  Findings, Completion Approval is recorded, and focused completion commit
  `e83e67b9` is complete.
- Audit findings and decisions:
  - Explorer `Header.tsx` has a transparent sticky `AppBar`, so scrolling
    ...
    `ExplorerTreePanel.tsx` also uses a transparent `Paper`, which makes the
    tree surface inherit page content. Both are actionable and will use one
    shared theme-aware opaque sticky/panel treatment with the existing MUI
    theme and forced-colors conventions.
  - Explorer `Header.tsx` and Calendar `ScheduleImpactCalendarFilters.tsx`
    use native selects with independent sizing. Explorer has one filter and
    Calendar has root, outcome, and run-state filters. They will use one
    browser-safe MUI `Select` wrapper with `autoWidth={false}` so the menu
    anchor matches the trigger, viewport-bounded Paper, wrapped long labels,
    and a non-selectable `ListSubheader` heading. Existing localized field
    labels are reused as headings, including the existing Japanese
    `変更を絞り込む`; no new filter meaning or locale key is introduced.
  - Calendar's sticky header is already opaque and the shared result cards,
    sections, comparison shell, one-row key/value list, and virtualization
    already provide the correct baseline. The implementation will normalize
    the Calendar header through the shared surface helper and preserve the
    result primitives unless focused audit assertions identify a concrete
    spacing, border, elevation, or overflow defect. Flow/Table's
    `semanticDiffViewerSurfaceSx` and Unit Definition's MUI form-control
    conventions are reference patterns only and are not changed by this
    slice.
  - Both contents shells, headers, filters, tree/panel surfaces, result cards,
    comparison sides, status chips, empty states, and bounded lists will be
    checked for spacing, surface contrast, border/elevation/z-index, narrow
    width overflow, long labels, light/dark/forced-colors rendering, focus,
    keyboard operation, and screen-reader names. This inventory keeps fixes
    limited to the listed presentation classes and does not redesign the
    result hierarchy or change schedule facts.
- User / Domain Value: Explorer and Calendar present the same predictable MUI
  control and surface behavior. Sticky content remains readable over scroll,
  filter menus explain their purpose, trigger and menu widths align, long
  English/Japanese labels wrap within the viewport, and keyboard, focus,
  screen-reader, light/dark, high-contrast, and narrow-layout behavior remain
  usable.
- Implementation boundary:
  - Add `ViewerFilterSelect.tsx` under the shared webview editor package. It
    owns only MUI `FormControl`, `InputLabel`, `Select`, `ListSubheader`, and
    `MenuItem` composition and shared menu sizing/wrapping/focus styles. It
    accepts plain string options and callbacks, exposes an explicit trigger
    ref, uses MUI `MenuProps`/slot props and default anchoring, and never
    performs data or schedule interpretation.
  - Add one exported theme-aware sticky surface style to `muiTheme.ts` and
    apply it to both headers; use an opaque `background.paper` surface,
    divider border, existing AppBar z-index, consistent elevation, and the
    existing Canvas/forced-colors behavior. Apply the same opaque panel rule
    to the Explorer tree Paper while leaving selected/transparent row states
    inside that panel unchanged.
  - Replace Explorer's filter and Calendar's three filters with the shared
    wrapper. Preserve existing IDs, values, option ordering, filter update
    functions, announcements, originating element focus, and localized labels.
    Widen the Calendar focus helper only as needed for the MUI trigger element;
    do not change filter state or model keys.
  - Keep the existing common `MyAppContextProvider`, `ThemeProvider`, global
    styles, resource path, host/session/transport, DTOs, result primitives,
    virtualization, and browser bundle unchanged. No custom theme listener,
    DOM menu positioning, or parallel view-specific filter implementation is
    allowed.
- Acceptance:
  - Explorer and Calendar sticky headers have opaque theme-aware surfaces with
    readable content above scrolling results; Explorer's tree panel is opaque;
    light, dark, high-contrast, and forced-colors assertions prove contrast,
    border, elevation, and z-index behavior without bespoke theme detection.
  - Each filter trigger has a stable accessible name and existing ID/value;
    every opened menu has the localized non-selectable field heading first,
    followed by the same option values and order as before. Heading text is
    excluded from selection and arrow navigation, and Escape, Enter, Home,
    End, and focus restoration retain MUI keyboard/a11y behavior.
  - Menu Paper width is anchored to its trigger (`autoWidth={false}`), remains
    within a narrow viewport, and wraps long root paths and localized labels.
    Responsive filter stacks do not create horizontal overflow. Existing
    filter announcements, result counts, no-match behavior, and Calendar
    Explorer action focus remain unchanged.
  - Shared result cards, sections, comparison before/after order, one-row
    label/value semantics, empty states, bounded lists, large collections,
    reduced motion, and malformed/long values retain their existing behavior;
    no schedule fact, DTO, protocol, ID, focus key, or virtualization contract
    changes.
  - The Unreleased changelog records one concise user-visible entry for the
    shared Explorer/Calendar MUI surface and filter consistency improvement.
- Planned paths (exact Slice 15 implementation scope):
  <!-- markdownlint-disable MD013 -->
  ...
  - `docs/specs/features/schedule-impact-calendar/TRACEABILITY.md`
  <!-- markdownlint-enable MD013 -->
- Validation-only references: `semanticDiffExplorerContents.tsx`,
  `ScheduleImpactCalendarContents.tsx`, shared `ResultCard.tsx`,
  `ResultSection.tsx`, `ResultComparison.tsx`, `ResultKeyValueList.tsx`,
  `ScheduleImpactCalendarBoundedList.tsx`, `sharedResultPresentation.test.tsx`,
  `architectureDependencyRules.test.ts`, Flow/Table contents, and
  `UnitDefinitionDialog.tsx` are included in the audit and regression checks;
  they are edited only if an actionable defect in this slice's listed classes
  is reproduced and independently scoped in review.
- Validation: run the new shared-select tests and focused Explorer component,
  DOM, theme-context, Calendar component, view, accessibility, and
  theme-context suites; run shared-result regression and architecture checks;
  run `rtk pnpm run test:compile`, production build, desktop/web builds and
  host/smoke checks. Compare `rtk pnpm run qlty` and
  `qlty smells --no-snippets` with the Slice 14/origin-main inventory; no
  suppression or quality configuration change is allowed. Run Markdown lint,
  `git diff --check`, and explicit light/dark/forced-colors, EN/JA,
  narrow-viewport, long-label, keyboard, focus, screen-reader, reduced-motion,
  and large/malformed-result checks.
- Quality delta: no Slice 15 runtime or test code exists in this replan. The
  current baseline is Slice 14's passing `qlty check` with only documented
  pre-existing advisory smells and no new finding; implementation must compare
...
- Dependencies and gate: Slice 15 depends on completion-committed Slice 14
  ...
  Completion Approval and focused completion commit `e83e67b9` are complete
  under the recorded gates. Slice 16 is now the next replan; Feature Exit is
  reopened and deferred.
- Risks: replacing native selects with MUI Select changes the DOM role and
  ...
  paths and translated labels may still challenge very narrow viewports;
  mitigate with trigger anchoring, viewport-bounded Paper, wrapping, and
  responsive stack tests. Sticky surface changes can hide top content or fail
  forced-colors contrast; verify z-index, border, and Canvas behavior. A new
  domain fact, protocol/message change, theme/resource mechanism, host
  lifecycle change, virtualization change, unrelated smell cleanup, or result
  primitive redesign returns to Main for Replanning.
- Out of scope: schedule semantics, result/DTO/JSON/report contracts, filter
  meaning or ordering, IDs and data attributes, host/session/transport,
  common resource/theme detection, Flow/Table/Unit Definition runtime changes,
  bundle/webpack changes, dependency upgrades, new locale meaning, roadmap
  closure, Feature Exit, and implementation/commit/approval in this replan.
...
- Plan gate status: reviewed and Human-approved; focused plan commit
  `cd4d932e` is complete. Implementation review, Completion Approval, and
  focused completion commit `e83e67b9` are complete under the recorded Slice
  15 boundary. Next stage: independent Feature Exit review is complete; final
  batch human Closure Approval is the sole remaining gate.

### Slice 15 Implementation Evidence

- Implementation status: Complete under focused plan commit `cd4d932e`;
  independent implementation review is `Ready` with no Findings, Completion
  Approval is recorded on 2026-09-20, and focused completion commit
  `e83e67b9` is complete.
...
- Completed paths are exactly the Slice 15 diff listed above. Focused
  completion commit `e83e67b9` is complete; Slice 16 is the next replan and
  Feature Exit is reopened and deferred.

### Slice 16: Canonical Shared Viewer Theme Configuration

- Status: Plan review is `Ready` with no Findings and
...
- Replanning trigger and audit evidence: the user questioned whether
  ...
  Calendar, and the shared filter, and is the single place preserving the VS
  Code font variables, palette mode, 44px interaction targets, and common
  Button/IconButton/Select focus and wrapping rules. The same webviews also
  import `editor/shared/viewerThemeStyles.ts`, which separately owns VS Code
  token borders, focus styles, and high-contrast global rules. The
  `semanticDiff*` names are therefore misleading shared exports; the
  `semanticDiffExplorerTheme` singleton has no production consumer, and the
  `MuiNativeSelect` override has no production NativeSelect consumer after
  Slice 15.
- User / Domain Value: all MUI webviews use one generic, browser-safe theme
  module whose name and exports describe viewer-wide behavior. Standard MUI
  `createTheme`, palette/background/elevation/z-index tokens, component
  `styleOverrides`, and local `sx` remain the implementation primitives. The
  refactor removes feature naming and duplicate ownership without changing
  colors, font variables, target sizes, focus visibility, forced-colors,
  high-contrast, control values, layout, or viewer behavior.
- Implementation boundary:
  - Move the generic theme factory and viewer styles into
    `src/presentation/webview/shared/viewerTheme.ts`. Rename exports to
    `createViewerTheme`, `ViewerThemeOptions`, `viewerTargetSizePx`,
    `viewerFocusSx`, `viewerSelectionSx`, `viewerGlobalStyles`,
    `viewerSurfaceSx`, `viewerOpaqueSurfaceSx`, and the existing generic
    `viewer*Border`/`viewerFocus*` helpers. Merge the two existing global-style
    objects with explicit precedence so the VS Code font, palette backgrounds,
    high-contrast Canvas tokens, forced-colors controls, Paper background
    reset, and focus outlines all remain covered.
  - Delete the old `muiTheme.ts` and `editor/shared/viewerThemeStyles.ts`
    after every internal consumer is migrated. Keep one `createTheme` call
    with the existing `MuiButton`, `MuiIconButton`, and `MuiSelect` shared
    overrides; remove only the unused `MuiNativeSelect` override and unused
    `semanticDiffExplorerTheme` singleton. Do not change the MUI dependency,
    the common MyAppContext/resource mechanism, or any theme detection.
  - Update Flow/Table/Explorer/Calendar and shared-control imports to the
    canonical module. Keep the Explorer direct-view and Calendar supplied-
    sidecar ThemeProvider seams, and use the generic global-style function in
    each app without changing the existing Flow minimap or loading states.
  - Consolidate the two theme test suites into `viewerTheme.test.ts`, update
    Explorer DOM and shared-filter tests to generic exports, and preserve
    assertions for light/dark palettes, VS Code tokens, font/global rules,
    focus/selection, 44px targets, opaque surfaces, and forced colors.
- Acceptance:
  - No production or test import references `muiTheme.ts`,
    `viewerThemeStyles.ts`, or `createSemanticDiffTheme`. The canonical
    module exports no feature-specific theme/surface/focus/selection/global
    style names and is shared by Flow, Table, Explorer, Calendar, and the
    filter control; no compatibility facade is left because the audit found
    only internal consumers.
  - The generic theme still uses standard MUI `createTheme` with the existing
    light/dark palette, VS Code font variables, shape, 44px Button/IconButton/
    Select controls, wrapped Select labels, focus-visible outlines, and
    forced-colors behavior. No live `NativeSelect` style is removed; only its
    unused theme override is deleted.
  - `viewerGlobalStyles` preserves html/body/root backgrounds and typography,
    VS Code high-contrast selectors, forced-colors Button/Select and focus
    rules, `.sde-state` contrast, and MUI Paper background reset. Existing
    opaque sticky/panel, selection, search, path, and panel borders resolve
    through MUI palette or VS Code variables as before.
  - Flow/Table/Explorer/Calendar production and direct-test shells compile
    and render with the same resource-derived light/dark mode, localization,
    focus behavior, filter values, result facts, and bundle names. DTOs,
    protocols, host lifecycle, virtualization, action IDs, and webview
    boundaries are unchanged.
- Planned paths (exact Slice 16 implementation scope):
  <!-- markdownlint-disable MD013 -->
  - `src/presentation/webview/shared/viewerTheme.ts` (new canonical module)
  ...
  - `src/presentation/webview/editor/shared/ViewerFilterSelect.tsx`
  - `src/test/suite/viewerTheme.test.ts` (new consolidated theme suite)
  ...
  - `docs/specs/features/schedule-impact-calendar/TRACEABILITY.md`
  <!-- markdownlint-enable MD013 -->
- Validation-only references: `src/test/suite/architectureDependencyRules.test.ts`,
  `webpack.config.js`, `package.json`, Flow/Table/Explorer/Calendar theme and
  resource-context suites, existing shared-result/accessibility suites, and
  locale files. They are not implementation paths unless an independently
  reproduced import or bundle regression requires a narrowly scoped update.
- Validation: run the consolidated theme, Explorer DOM, shared-filter,
  Flow/Table theme, Calendar view/theme-context, shared-result, accessibility,
  and architecture suites; run `rtk pnpm run test:compile`, production
  webpack, desktop preparation/smoke, web bundle and WEB-7–10 host/smoke
  checks. Verify light/dark/high-contrast/forced-colors, VS Code font tokens,
  44px target and focus rules, opaque surfaces, Flow minimap styles, direct
  Explorer/Calendar test seams, narrow/long-label filter behavior, and no
  old-module imports. Run `rtk pnpm run qlty`, the smell delta against
  `e83e67b9`/`origin/main` with no suppression or configuration change,
  Markdown lint, and `git diff --check`.
- Production readiness and documentation: this is behavior-preserving
  internal cleanup, so `CHANGELOG.md`, `SPECS.md`, `docs/specs/roadmap.md`,
  locales, webpack entry/output names, and durable use-case documentation are
  review-only and remain unchanged. No Node, VS Code, domain, application,
...
- Risks: merging global style objects can change CSS precedence, deleting the
  ...
  guard. Mitigate with complete repository reference search, canonical export
  assertions, computed palette/forced-colors tests, direct view-shell tests,
  desktop/web builds, and an import/bundle guard. Any external consumer,
  required public compatibility facade, or behavior change returns to Main for
  Replanning rather than adding a silent alias.
...
- Human Plan Approval: Approved on 2026-09-20 under the user's standing
  automatic no-findings slice approval instruction.
- Approved boundary: canonicalize the shared browser-safe MUI viewer theme
  and generic style exports, migrate the listed Flow/Table/Explorer/Calendar
  and shared-control consumers/tests, remove only the unused NativeSelect
  override and singleton, and preserve all theme/resource, accessibility,
  DTO, protocol, host, virtualization, and bundle behavior. `CHANGELOG.md`,
  `SPECS.md`, `docs/specs/roadmap.md`, locales, webpack configuration, and
  unrelated runtime/test paths are outside this approval.
- Approved paths (exact Slice 16 plan scope):
  <!-- markdownlint-disable MD013 -->
  - `src/presentation/webview/shared/viewerTheme.ts` (new canonical module)
  - `src/presentation/webview/shared/muiTheme.ts` (deleted)
  - `src/presentation/webview/editor/shared/viewerThemeStyles.ts` (deleted)
  ...
  - `src/presentation/webview/editor/shared/UnitTreeSelector.tsx`
  - `src/presentation/webview/editor/shared/ViewerFilterSelect.tsx`
  - `src/test/suite/viewerTheme.test.ts` (new consolidated suite)
  - `src/test/suite/muiTheme.test.ts` (deleted)
  - `src/test/suite/viewerThemeStyles.test.ts` (deleted)
  - `src/test/suite/semanticDiffExplorerDom.test.tsx`
  - `src/test/suite/viewerFilterSelect.test.tsx`
...
- The dependency chain is strict: schedule/structured-output predecessor
  ...
  before/after comparison layout Slice 12 → one-row result alignment and
  targeted smell cleanup Slice 13 → internal-ID presentation cleanup Slice 14
  → shared MUI presentation audit Slice 15 → canonical shared MUI viewer-theme
  cleanup Slice 16.
  Slice 2 must not
  expose a user-reachable action; Slice 3 remains the first public surface and
  Slices 4–16 preserve it.
...
- Replanning is required for any predecessor contract/JSON/report/Explorer
  ...
  feature-author decision. A change from Slice 10's report-package ownership
  boundary into report semantics, JSON schema, mode behavior, or public output
  APIs also requires Replanning. A change from Slice 11's presentation-only
  primitives into localized domain formatting, DTO/protocol behavior, host
  lifecycle, or a separate theme mechanism also requires Replanning.
  Slice 16's generic theme-module boundary does not authorize a new
  theme/resource mechanism, MUI dependency change, user-control redesign, or
  public compatibility facade.

## Traceability

- `TRACEABILITY.md` is required and maps CAL requirements to the sixteen slices,
...
- The sidecar identity, composite source-change resolution, root matrix,
  ...
  range formatting and before/after comparison layout are Slice 12-owned;
  one-row shared result alignment and targeted Slice 11/12 smell cleanup are
  Slice 13-owned; internal Calendar-ID presentation cleanup is Slice 14-owned;
  shared MUI surface/filter consistency and responsive accessibility are Slice
  15-owned; canonical shared viewer-theme configuration and export cleanup are
  Slice 16-owned.
  Feature Exit is reopened and no requirement is treated as closed until
  Slice 16 completes its gates.
...
- Production readiness: desktop and web checks, compatibility guards,
  ...
  entry is applied. Slice 12's additional range/comparison implementation is
  reviewed `Ready` with no Findings, automatic Completion-approved, and focused
  completion commit `2fb18daa` is complete. Existing macOS codesign,
  web-stream cleanup, webpack-size, and advisory smell findings remain
  documented observations. Slice 13's one-row result alignment and targeted
  smell cleanup are implemented under focused plan commit `a314dc54`; its
  ...
  `b8d9a667` is complete.
- Slice 14's internal-ID cleanup is independently reviewed `Ready` with no
...
- Slice 15's shared MUI presentation audit is independently reviewed `Ready`
  with no Findings, Completion-approved, and focused completion commit
...
- Proposed closure scope after Slice 16: retain the prepared
  ...
  review and explicit Closure Approval; inherited feature folders remain
  preserved.
...
- [x] Slice 6 package-relocation replan received independent `plan-reviewer`
      `Ready` review with no Findings and was approved in focused commit
      `e51d6def`. Its host/browser path moves, canonical import updates,
      obsolete facade removal, location guard, CSP/output-name checks, and
      full validation are complete.
- [x] Slice 6 received independent `implementation-reviewer` `Ready` review
      with no Findings. Main recorded automatic Completion Approval under the
      user's no-findings instruction; focused implementation commit `c36ee1cf`
      is complete. Its direct 74-test matrix, architecture/location 26-test
      suite, independent 56-test UI subset, compile, build, host, quality,
...
- [x] Slice 8 shared Calendar-resource/theme replan received independent
      ...
      Findings, automatic Completion Approval was recorded on 2026-09-19, and
      focused completion commit `fe042fb0` is complete. Calendar matrix 26,
      architecture 26, compile/build/host/quality/lint/diff checks pass.
- [x] Slice 9 placement-policy replan received independent `plan-reviewer`
      `Ready for approval` review with no Findings and
      ...
      exact paths above. Focused plan commit `65f123ee` is complete.
- [x] Slice 9 implementation received independent
      `implementation-reviewer` `Ready` review with no Findings. Main recorded
      automatic Completion Approval on 2026-09-19 under the user's standing
      no-findings instruction for the exact completed paths below. Focused plan
      commit `65f123ee` is complete; focused completion commit `bb8d7305` is
      complete. Architecture 27, focused Calendar bridge/session/transport 14,
      compile/build/host/smoke WEB-7–10/quality/lint/diff checks pass.
- [x] Slice 10 Presentation-report replan received independent
      ...
      recorded on 2026-09-19. Focused completion commit `0a5cdd31` is complete;
      architecture 28, picker 2, JSON 17, compile/build/host/smoke WEB-7–10,
      quality/lint/diff checks pass.
- [x] Slice 11 shared-result presentation plan received independent
...
- [x] Slice 12 localized-range/comparison-layout plan received independent
      ...
      `Replanning required: No`; Human Plan Approval was recorded on
      2026-09-20 under the user's automatic no-findings instruction for the
      exact paths above. Focused plan commit `b25e38d5` is complete and the
      implementation is complete under the approved paths. Independent
      ...
      focused plan commit `a314dc54`, independently reviewed `Ready` with no
      Findings, Completion-approved, and focused completion commit `b8d9a667` is
      complete. Focused validation evidence is recorded in TRACEABILITY.
- [x] Slice 13 one-row result alignment and targeted smell cleanup plan
      ...
      Findings, Completion Approval was recorded on 2026-09-20, and focused
      completion commit `b8d9a667` is complete.
- [x] Slice 14 internal-ID presentation cleanup has an independent
      `plan-reviewer` `Ready` result with no Findings and Human Plan Approval
      recorded on 2026-09-20. Focused plan commit `9a0bd721` and implementation
      are complete under the approved paths; independent implementation review
      returned `Ready` with no Findings, Completion Approval was recorded on
      2026-09-20, and focused completion commit `52ede211` is complete. The
...
- [x] Slice 15 full Explorer/Calendar MUI presentation audit is implemented
...
- The public action is exposed only for a successful evaluated-period artifact
  ...
  theme/context Slice 8 for Calendar dependency chain, Slice 9 placement
  policy, Slice 10 report-package organization, shared result-presentation
  Slice 11, localized range/comparison Slice 12, and planned one-row result
  alignment/targeted smell cleanup Slice 13, and internal-ID presentation
  cleanup Slice 14, the Slice 15 shared MUI presentation audit, and the Slice
