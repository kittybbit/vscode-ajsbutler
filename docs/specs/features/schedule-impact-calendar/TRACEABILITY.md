...
| `CAL-ZERO-001`: distinguish explicit valid no-runs from partial, unsupported, invalid, and uncalculated outcomes       | Requirements: CAL-ZERO-001; Root Outcomes, Timeline, Issues, And Filters; Acceptance Criteria             | Slices 1 and 3 | `semanticDiffScheduleRules.test.ts` and `semanticDiffScheduleImpact.test.ts`: supported/valid-no-runs/partial plus invalid, `missing-context`, and unsupported status matrix proves no zero-run relabeling; null-side, zero-only, mixed, and malformed cases |
| `CAL-UNKNOWN-001`: keep unsupported and uncalculated schedule portions visible with stable evidence                    | Requirements: CAL-UNKNOWN-001; Root Outcomes, Timeline, Issues, And Filters; Acceptance Criteria          | Slices 1 and 3 | `semanticDiffScheduleRules.test.ts` and `semanticDiffScheduleImpact.test.ts`: same calendar reason with upstream `missing-context`/`invalid`/`unsupported` status remains distinguishable; issue-code/detail preservation, side/root references, uncalculated section, no prose inference, malformed and missing-context cases |
...
- Slice 1 uses the schedule-semantics predecessor evaluation and the
  ...
  owner must provide an additive, evaluation-only carrier for the already
  computed `SemanticDiffScheduleStatus` on unsupported-decision records; a
  separate owner decision and Human Approval are required before
  implementation. `SemanticDiffResult`, reports, and JSON version 1 remain
  unchanged.
