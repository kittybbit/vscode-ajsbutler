...
- Approved or active slice: all five implementation slices are implemented and
  independently reviewed Ready with no findings. Slices 1–4 are
  completion-committed as `4a3b846d`, `23dfdcea`, `e9f25f69`, and `197d062d`,
  respectively. Slice 5 Completion Approval was automatically granted on
  2026-09-06 under the user's explicit per-slice instruction; its completion
  commit is pending. Feature-level final/closure approval remains pending
...
- Next decision: send the exact Slice 5 diff to approval-committer for its
  completion commit. Feature-level final/closure approval remains pending
  until Feature Exit and the requested bulk human approval.
...
- Review status: Ready; all five implementation-reviewer verdicts are Ready
  with no findings.
- Human approval: Existing plan and Slice 2 replan approved; replan commit
  `cb17d26a`
- Active implementation slice: none; all five slices are implementation
  complete and reviewed. Slice 5 completion commit is pending.
...
- Status: Complete and committed as `23dfdcea`.
...
- Implementation review verdict: Ready; no findings.
- Commit status: Complete and committed as `23dfdcea`.
- Feature-level final/closure approval: Pending until Feature Exit; the user
  requested one bulk human approval after all five slices are complete.
...
- Approval effect: Human Approval for this replan was recorded on 2026-09-05
  ...
  implementation and correction were later completion-committed as
  `23dfdcea`. The correction was independently reviewed Ready with no
  findings, and the Slice 2 Completion Approval is recorded above.
...
- Scope note: no other runtime or test path is authorized by this replan. The
  existing Slice 2 interpreter/projector, application, documentation, and
  integration-test changes were carried forward unchanged; the approved
  correction was implemented and included in the Slice 2 diff, which was
  completion-committed as `23dfdcea`.

## Slice 3 Completion Approval

- Status: Complete and committed as `e9f25f69`; implementation-reviewer Ready
  with no findings; Completion Approval granted automatically on 2026-09-06
  under the user's explicit per-slice instruction.
- Approved at: 2026-09-06 automatically under the user's explicit per-slice
  instruction that a slice is approved when independent implementation review
  has no findings.
- Approved scope: the reviewed Slice 3 implementation and its exact current
  path set below; no other slice or feature-level closure work is approved by
  this record.
- Approved paths: `CHANGELOG.md`, `README.md`,
  `docs/requirements/domain-rules/interpret-jp1-parameters.md`,
  `docs/requirements/use-cases/uc-build-semantic-diff.md`,
  `docs/specs/features/schedule-semantics-expansion/TASKS.md`,
  `docs/specs/features/schedule-semantics-expansion/TRACEABILITY.md`,
  ...
  `src/test/suite/semanticDiffScheduleRules.test.ts`.
- Implementation review verdict: Ready; no findings.
- Commit status: Complete and committed as `e9f25f69`.
- Feature-level final/closure approval: Pending until Feature Exit; the user
  requested one bulk human approval after all five slices are complete.
...
- Acceptance:
  ...
  - Existing `cy`, `sh`, `shd`, `ln`, `cftd`, relative `st`, day-crossing `st`,
    ...
    makes an `sc`-only unit schedule-defined. At the Slice 1 boundary, `jc`
    was classified as `schedule` but its resolution was intentionally deferred;
    Slice 3 now resolves its normalized calendar context.
...
- Status: Complete and committed as `23dfdcea`; focused replan approved and
...
- Dependencies: Slice 1 complete and committed; Slice 2 implementation and
  approved correction are complete and committed as `23dfdcea`.
...
- Carried-forward implementation paths were the two schedule domain modules,
  the application schedule suite, `README.md`, `CHANGELOG.md`, and the shared
  rule/use-case documents. The complete Slice 2 diff was committed as
  `23dfdcea`; its approved correction paths are recorded in `Slice 2 Replan
Approval` above.
...
- Implementation evidence: Slice 3 resolves each jobnet's selected absolute
  `jc` group or nearest containing `unitType=g` ancestor from the full
  normalized side document. Closest explicit `sdd`, `md`, and `stt` values are
  applied with defaults 1, `th`, and `00:00`; duplicate, malformed, cyclic, or
  ambiguous context remains recoverable evidence.
- Projection evidence: fully qualified `+DD`, `+b`, `+b-DD`, and relative
  weekday forms use definition-backed operational-month intervals for `md=th`
  and `md=ne`. Missing/non-group `jc`, non-zero base time, impossible base
  boundaries, and absent context remain `calendar-selection` evidence. The
  existing run, unsupported-item, zero-run, and DTO shapes are unchanged.
- Condition evidence: only fully qualified, syntactically valid relative dates
  are promoted to calendar-context resolution. Omitted-year/month forms retain
  `unsupported-schedule-date`, while invalid relative offsets or weekday
  occurrences retain `invalid-calendar-day` even when context is absent.
- Evidence preservation: every duplicate `sdd`, `md`, and `stt` parameter is
  retained in domain raw evidence before the existing application mapping is
  applied.
- Application evidence: schedule comparison receives full before/after
  normalized documents for context resolution while continuing to pass only
  scoped units to comparison; units in an out-of-scope `jc` group are never
  added to the result.
- Changed paths (exact current Slice 3 diff): `CHANGELOG.md`, `README.md`,
  `docs/requirements/domain-rules/interpret-jp1-parameters.md`,
  `docs/requirements/use-cases/uc-build-semantic-diff.md`,
  `docs/specs/features/schedule-semantics-expansion/TASKS.md`,
  `docs/specs/features/schedule-semantics-expansion/TRACEABILITY.md`,
  ...
  `src/test/suite/semanticDiffScheduleCalendar.test.ts`, and
  `src/test/suite/semanticDiffScheduleRules.test.ts`.
- Scaling evidence: each side builds one iterative document index and passes it
  to per-unit resolution. A 144-rule ten-year schedule with 512 unrelated
  groups remains bounded to the requested target runs.
- Application mapping evidence: invalid/missing `jc`, duplicate base,
  hierarchy-cycle, impossible-boundary, and non-zero-`stt` cases assert the
  existing dynamic unsupported IDs, reason codes, legacy messages, and raw
  parameter details.
- Validation evidence: focused calendar-context, relative-projection, and
  ...
  `MachPortRendezvousServer` permission failure.
- Compatibility impact: Gregorian arithmetic is pure and browser-safe, with
  no host locale, timezone, clock, filesystem, network, WebAPI, or external
  calendar acquisition. Existing direct dates and public/neutral DTOs remain
  unchanged.
- Production readiness: only operational months requested by relative rules
  are constructed; missing, invalid, duplicate, cyclic, and impossible context
  is explicit rather than guessed. Open/closed counting, substitution,
  inheritance, 48-hour, cycle, and other deferred semantics remain outside
  this slice.
- Implementation review: Ready with no findings. Completion Approval was
  granted automatically on 2026-09-06 under the user's explicit per-slice
  instruction, and completion commit `e9f25f69` is recorded. Feature-level
  final/closure approval remains pending until Feature Exit and the requested
  bulk human approval. Slice 4 is also committed as `197d062d`.

### Slice 4: Project Explicit Open And Closed Days

- Status: Complete and committed as `197d062d`; implementation-reviewer Ready
  with no findings; Completion Approval automatically granted on 2026-09-06
  under the user's explicit per-slice instruction
...
- Implementation evidence: `op` and `cl` values from the selected normalized
  group and its ancestors are resolved without host or external calendar
  lookup. The closest matching group is used independently for exact-date and
  weekday selectors; an exact-date selector takes precedence for its date.
  Identical duplicate values are idempotent, while same-group open/closed
  conflicts remain invalid with `schedule:calendar:invalid-base-or-conflict:*`
  evidence.
- Projection evidence: fully qualified `*DD`/`@DD` forms count classified days
  forward from the operational-month start, and `*b[-DD]`/`@b[-DD]` count them
  backward from the end with the documented `0..34` backward-offset range for
  open/closed forms. Each inspected date must be explicitly classified;
  incomplete context remains `missing-context`/`calendar-selection`. A month
  with no qualifying day produces a valid no-runs result, including the
  existing zero-run confirmation path.
- Application evidence: existing `calendar-selection` reason codes, dynamic
  unsupported IDs, legacy messages, and raw schedule detail remain unchanged;
  no public or neutral DTO, confirmation policy, command, or presentation
  contract was added. Before/after definition contexts remain isolated and
  out-of-scope calendar groups do not widen comparison.
- Validation evidence: focused calendar, schedule-rule, and application tests,
  ...
  host Chromium `MachPortRendezvousServer` permission failure. The focused
  suite includes a complete 35-day operational month covering open/closed
  backward offsets 31 and 34, with offset 35 retained as invalid.
- Compatibility and production readiness: the resolver and projector use
  pure Gregorian arithmetic and normalized definition data only, with no
  locale, timezone, clock, filesystem, network, WebAPI, or service-calendar
  dependency. Invalid, incomplete, and no-run outcomes remain recoverable and
  explicit. Substitution, omitted-`sh`, inheritance, cycles, 48-hour time, and
  `cftd` remain deferred.
...
- Status: Implementation complete; implementation-reviewer Ready with no
  findings; Completion Approval automatically granted on 2026-09-06 under the
  user's explicit per-slice instruction; completion commit pending.
...
- Implementation evidence: rule-associated `sh`/`shd` values are normalized in
  the domain projector. `be` and `af` select the nearest definition-backed open
  day within the effective limit (default `2`, explicit `1..31`); `ca` keeps an
  open base date and suppresses a closed one. Identical duplicates are
  idempotent, conflicts and unpaired/invalid values retain raw evidence, and
  `sh=no` remains missing context. Candidate lookaround is bounded to 31 days
  on each side before final half-open period filtering. Rules containing `cy`
  or `cftd` do not receive partial substitution; this whole-rule suppression
  derives rule 1 by default (or an explicit raw rule prefix) even when the
  unsupported parameter is malformed and cannot be parsed.
  Substitution is gated to fully qualified `sd` dates; legacy omitted-year or
  omitted-month `MM/DD` and `DD` forms retain their direct-date compatibility,
  while their `sh` substitution remains uncalculated with the existing
  `closed-day-substitution` evidence.
- Application evidence: existing `closed-day-substitution` and `shift-days`
  mappings, legacy messages, dynamic item IDs, raw details, run-change shape,
  zero-run confirmation, and public/neutral DTOs remain unchanged. Focused
  tests cover before/after shifts, default and explicit limits, no target,
  period crossing, `ca`, duplicate raw evidence, invalid/unresolved values,
  cycle interaction, malformed `cy`/`cftd` whole-rule suppression, and the
  omitted-year/month substitution boundary. Duplicate substitution rules
  retain four per-parameter raw evidence entries, and conflicting `sh` values
  retain one unsupported decision per raw parameter.
- Compatibility and production readiness: substitution uses only the existing
  normalized calendar context and browser-safe Gregorian date arithmetic. No
  host locale, timezone, clock, filesystem, network, WebAPI, or scheduler
  service state is consulted. Missing or invalid context remains recoverable.
- Validation evidence: focused schedule/calendar/application tests,
  `rtk pnpm run test:compile`, desktop extension tests, an escalated
  `rtk pnpm run qlty` (`qlty check`: No issues), `rtk pnpm run lint:md`,
  `rtk pnpm run build`, and `git diff --check` are green. The normal sandbox
  qlty invocation cannot create its external rolling log, so the successful
  implementer run used the approved external-log access. `rtk pnpm run
test:full` passes its desktop portion; the Web portion remains blocked
  before tests load by the unchanged host Chromium
  `MachPortRendezvousServer` permission failure.
- Changed paths (exact current Slice 5 diff): `CHANGELOG.md`, `README.md`,
  ...
  `src/test/suite/semanticDiffScheduleCalendar.test.ts`.
- Status: Implemented; implementation-reviewer Ready with no findings;
  Completion Approval automatically granted on 2026-09-06 under the user's
  explicit per-slice instruction; completion commit pending. Feature-level
  final/closure approval remains pending until Feature Exit and the requested
  bulk human approval.
