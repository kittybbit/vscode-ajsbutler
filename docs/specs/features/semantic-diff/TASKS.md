...
### Slice 11: Localized Semantic Diff Report

- Status: Complete
- Replanning Gap: Feature Exit review found the command workflow complete, but
  generated semantic diff report wording remains English-only even when the
  host display language is Japanese.
- Smallest Useful Slice: delivers one user-facing report value: users working
  in a Japanese VS Code environment can review semantic diff output in
  Japanese while preserving English fallback.
- Scope: add report-language support to semantic diff Markdown rendering,
  pass the VS Code display language from the command/report application path,
  and localize generated report wording for Japanese. Where generated wording
  is currently stored as English-only `summary`, `rationale`, `changeContent`,
  or `message` strings, add stable semantic message codes/parameters or
  equivalent structured DTO fields inside the semantic diff boundary so the
  renderer can generate English and Japanese wording from the same comparison
  result data. Generated wording includes headings, section labels, empty
  states, structural change summaries, matching rationale,
  confirmation-required items, unsupported and limitation notes, schedule
  wording, count labels, and before/after labels. Semantic identifiers, paths,
  parameter keys, relation types, raw JP1/AJS values, and parser-provided raw
  messages stay unchanged.
- User / Domain Value: Japanese users can read the report without translating
  the extension-generated explanation text, while reviewers still see stable
  JP1/AJS identifiers and raw values.
- Cohesive Change Group:
  - likely files:
    `src/domain/models/semantic-diff/SemanticDiff.ts`,
    `src/application/semantic-diff/compareSemanticDiff.ts`,
    `src/application/semantic-diff/compareScheduleDiff.ts`,
    `src/application/semantic-diff/renderSemanticDiffMarkdown.ts`,
    `src/application/semantic-diff/buildSemanticDiffReport.ts`,
    `src/presentation/vscode/commands/semanticDiffCommand.ts`
  - likely tests:
  ...
  - related existing files:
    ...
    `src/presentation/vscode/commands/importAjsDefinitionViaWebApiCommand.ts`
- Acceptance:
  - report rendering accepts a language option and defaults to English when no
    language is supplied
  - `ja` and `ja-*` language values render generated Markdown report wording in
    Japanese
  - unsupported language values fall back to the current English report wording
  - the VS Code command uses `vscode.env.language` when building the report
  - structural changes, matching rationale, confirmation-required items,
    unsupported items, limitation notes, and schedule changes no longer depend
    on prebuilt English-only summaries for Japanese output
  - confirmation-required, unsupported, limitation, and schedule report
    wording is localized where the wording is generated by semantic diff code
  - semantic identifiers, unit paths, parameter keys, relation types, and raw
    JP1/AJS values are not translated
  - comparison DTO meaning, semantic change IDs/kinds/targets, matching
    decisions, schedule calculations, report display, and explicit copy
    behavior remain unchanged
- Validation:
  - renderer tests for English default, `ja`, regional `ja-JP` Japanese output,
    and unsupported language fallback such as `fr`
  - focused tests that Japanese structural change output localizes generated
    summaries/rationale while preserving raw identifiers and values
  - renderer tests that parser-provided limitation or unsupported-item messages
    remain verbatim in Japanese output while semantic-diff-generated wording is
    localized
  - comparison tests or sample assertions that semantic change IDs, kinds,
    targets, confirmation-required categories, unsupported items, limitations,
    and schedule decisions are unchanged by the localization refactor
  - command/build-report tests that verify `vscode.env.language` is passed to
    report rendering
  - sample coverage assertion that the reusable semantic diff sample can render
    Japanese report wording from the same semantic result data without
    changing comparison categories
  - `rtk pnpm run qlty`
  - `rtk pnpm test`
  - `rtk pnpm run test:web`
  - `rtk pnpm run build`
- Production Readiness:
  - Failure mode: missing or unsupported language values fall back to English
    rather than failing report generation.
  - JP1/AJS compatibility: no parser, normalization, comparison, schedule, or
    JP1/AJS interpretation changes; raw identifiers and values are preserved.
  - Large or malformed input risk: localization must reuse deterministic
    rendering paths and avoid extra comparison passes.
  - Desktop/web impact: language selection uses host-provided language at the
    VS Code boundary and keeps renderer logic host-neutral.
  - README/docs impact: update usage wording only if user-facing report
    localization needs to be documented.
  - CHANGELOG impact: required because report output language is externally
    observable behavior.
- Approval Boundary: report-language DTO/input option, stable semantic
  message-code/parameter or structured-message additions inside
  `SemanticDiff` DTOs, focused updates to semantic diff comparison modules
  needed only to replace generated English strings with localizable
  descriptors, Markdown localization tables or helpers, command/build-report
  language wiring, focused tests, and README/CHANGELOG wording if needed for
  the localized report behavior.
- Dependencies: Slices 3, 4, 9, and 10. It depends on the Markdown renderer,
  command report workflow, displayed report surface, and sample coverage.
- Risks:
  - Localizing prebuilt English `summary`, `rationale`, and `message` strings
    by string matching would be brittle; prefer rendering generated wording
    from semantic DTO fields or introducing stable message codes/parameters
    inside the semantic diff boundary.
  - Changing DTO representation can accidentally alter downstream report,
    sample, or flow-highlight assumptions; tests must prove semantic IDs,
    kinds, targets, and decisions stay stable while only generated wording
    changes by language.
  - Over-localizing JP1/AJS identifiers, parameter keys, or raw values would
    reduce review accuracy; only generated explanatory wording is localized.
  - A broad i18n framework refactor would exceed this slice; keep localization
    scoped to semantic diff report wording.
- Out of Scope: translating command titles or VS Code manifest strings,
  translating JP1/AJS raw definition values, adding language preferences,
  localizing unrelated diagnostics/hover/table/flow UI, dedicated visual diff
  UI, telemetry, and changing comparison semantics.
...
- Scope: refactor only semantic-diff Markdown localization/rendering helpers
  and their focused tests so the existing report output, language fallback,
  deterministic ordering, semantic DTO meanings, and raw JP1/AJS values stay
  unchanged. Do not change comparison algorithms, command flow, report
  display/copy behavior, or resource message content except where a helper
  interface requires mechanical key access.
- User / Domain Value: report localization remains maintainable as additional
  semantic result categories are added, without increasing comparison or host
  complexity.
- Cohesive Change Group:
  - likely files: `src/application/semantic-diff/renderSemanticDiffMarkdown.ts`,
    a focused neighboring localization helper module if extraction improves
    readability, and `src/test/suite/renderSemanticDiffMarkdown.test.ts`
  - related files: `src/domain/services/i18n/nls.ts` and
...
- Validation:
  ...
  - `rtk pnpm run qlty`, `rtk pnpm test`, `rtk pnpm run test:web`, and
    `rtk pnpm run build`
- Production Readiness:
  - Failure mode: missing/unsupported language still resolves through existing
    English fallback and never prevents report generation.
  ...
  - CHANGELOG impact: not required because this is behavior-preserving.
- Approval Boundary: renderer/localization helper refactor, focused tests, and
  SDD traceability only. No comparison logic, VS Code command, manifest,
  telemetry, parser, flow, or unrelated Qlty-smell changes.
- Dependencies: Slice 11 implementation state.
- Risks: extracting generic abstractions may obscure report ordering or
  language fallback; retain report-oriented names and exact output tests.
- Out of Scope: all Qlty findings outside the semantic-diff Markdown renderer,
  new report wording, new languages, DTO redesign, and behavior changes.
...
- Slice 11 depends on Slices 3, 4, 9, and 10 because it localizes the existing
  Markdown report, uses command language wiring, preserves displayed-report
  copy behavior, and should be verified against the reusable sample.
- Slice 12 depends on Slice 11 because it preserves the localized report
  behavior while refactoring only its renderer-local complexity.
...
- Reason: feature changes user-visible behavior, JP1/AJS definition-file
  ...
  approvable slices.
- Status: Slice 11 validation is recorded; Slice 12 is proposed and requires
  plan revision and review before approval.
...
- Localized report wording can drift from English behavior; Slice 11 must keep
  both language outputs generated from the same semantic DTO data and validate
  English fallback.

## Use-Case Back-Propagation

- `docs/requirements/use-cases/uc-compare-semantic-diff.md` is updated for the
  additional displayed-report, explicit-copy behavior, and localized report
  wording. Revisit it only if review changes the approved report-surface or
...
- Definition of Done status: Slice 11 is complete. Feature Exit Review is
  blocked until Slice 12 is revised, reviewed, approved, implemented,
  validated, and completed or explicitly removed from scope.
...
- Open risks: broad JP1/AJS schedule semantics beyond the Slice 7 supported
  subset, report display lifecycle, localized report wording drift, sample
  maintainability, and feature exit documentation propagation.
