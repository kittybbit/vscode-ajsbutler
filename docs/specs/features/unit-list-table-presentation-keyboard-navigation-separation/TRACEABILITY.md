...
| `uc-view-unit-list.md`: keyboard movement changes focus without intermediate selection side effects; Enter and explicit handoffs commit the focused row | R5, R8; AC3, AC5, AC7            | Slice 4              | Remove `tableSelectionTiming.test.ts`; add cases to `tableNavigation.test.ts`, `tableVirtualizationFocus.test.ts`, `tableShellIntegration.test.ts`, and `accessibilityDom.test.tsx`; run `revealUnit.test.ts`, desktop/web/build, and manual large-list smoke |
| Roadmap 7.3: separate rendering, column actions, virtualization, keyboard focus, and keyboard-selection side-effect timing from Application DTOs        | Purpose; R1-R4, R8; AC1-AC3, AC7 | Slices 1-4           | architecture dependency test, focused table suites, `rtk pnpm run qlty`, production build, manual large-list keyboard smoke                                                                                                                                   |
