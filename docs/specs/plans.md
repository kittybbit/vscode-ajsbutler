...
2. Select the next Qlty-driven architecture refactoring phase outside
   completed Slice-3 domain-helper work.
...
- `docs/specs/features/qlty-driven-architecture-refactoring/`:
  ...
  Slice-3-G implementation is complete for `parameterHelpers.ts` parameter
  resolution helper cleanup. Slice-3-H investigation is complete for
  `scheduleRuleHelpers.ts` schedule-by-days parser helper cleanup and
  implementation is complete. Slice-3-I investigation is complete for
  `optionalScalarParameterBuilders.ts` optional scalar builder helper cleanup
  and implementation is complete. Slice-3-J investigation is complete for
  `AjsDocument.ts` inherited parameter lookup helper cleanup and
  implementation is complete. Slice-3-K investigation is complete for
  `relations.ts` normalized relation resolver cleanup and implementation is
  ...
  cleanup and implementation is complete. Slice-3-O investigation is complete
  for `Mqwj`/`Mswj` message-queue wait job shared getter cleanup and
  implementation is complete. Slice-3-P investigation and implementation are
  complete for `Fxj`/`Mlwj`/`Ntwj` wait-job shared getter cleanup. Slice-3-Q
  investigation is complete for `Evsj` event-sending shared execution getter
  cleanup, and implementation is complete. Slice-3-R investigation and
  implementation are complete for `Pwrj` power-control shared execution
  getter cleanup. Slice-3 domain-helper work is complete; remaining getter
  duplication is shape-only and should not be forced into another slice
  without a stronger semantic target.
