...
2. Select and request approval for Qlty-driven architecture refactoring
   Slice-1B-M: extract `revealVisibleNestedUnit` nested reveal helpers while
...
- `docs/specs/features/qlty-driven-architecture-refactoring/`:
  ...
  `appendExpandedUnitEdges` extraction. Slice-1B-M targets
  `revealVisibleNestedUnit` and needs separate approval.
